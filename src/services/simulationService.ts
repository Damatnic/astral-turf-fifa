...
export const simulateMatch = (
  ...
  for (let minute = 1; minute <= 90; minute++) {
    ...
    const homeTacticMods = {
      ...
      defensiveAction:
        (TACTIC_MODIFIERS.mentality as any)[homeTactics.mentality]?.defensiveAction ?? 1.0,
    ...
    const awayTacticMods = {
      ...
      defensiveAction:
        (TACTIC_MODIFIERS.mentality as any)[awayTactics.mentality]?.defensiveAction ?? 1.0,
    ...
    if (Math.random() < chanceCreation) {
      ...
      const attacker =
        attackers
          .filter(p =>
            ['MF', 'FW'].includes(PLAYER_ROLES.find(r => r.id === p.roleId)?.category ?? '')
          )
      ...
      if (Math.random() < goalProbability) {
        ...
        if (potentialAssisters.length > 0 && Math.random() < 0.7) {
          const assistPlayer =
            potentialAssisters[Math.floor(Math.random() * potentialAssisters.length)];
        ...
        const defender =
          defenders
            .filter(p =>
              ['DF', 'MF'].includes(PLAYER_ROLES.find(r => r.id === p.roleId)?.category ?? '')
            )
...
export const calculateMatchOutcome = (
  ...
  // Tactical modifiers
  const homeMentality = (TACTIC_MODIFIERS.mentality as any)[homeTactics.mentality] ?? {
    goalChance: 1.0,
    defensiveAction: 1.0,
  };
  const awayMentality = (TACTIC_MODIFIERS.mentality as any)[awayTactics.mentality] ?? {
    goalChance: 1.0,
    defensiveAction: 1.0,
  };
