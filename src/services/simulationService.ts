...
import type { Player, TeamTactics, MatchEvent, Team, PlayerRole, PositionRole, PlayerStats, RootState, MentoringGroup, MatchCommentary, MatchResult } from '../types';
...
const getTeamStrengths = (
    ...
    players.forEach(p => {
        ...
        if (role.category === 'FW') {
            attackSum += ((p.attributes.shooting * 1.2) + p.attributes.dribbling + p.attributes.positioning) / 3 * formMoraleMod * staminaMod;
            attackCount++;
        }
        if (role.category === 'MF') {
            midfieldSum += ((p.attributes.passing * 1.1) + p.attributes.positioning + p.attributes.dribbling + p.attributes.speed) / 4 * formMoraleMod * staminaMod;
            midfieldCount++;
            if (['dm', 'dlp', 'b2b'].includes(p.roleId)) {
                 defenseSum += ((p.attributes.tackling * 1.1) + p.attributes.positioning) / 2 * formMoraleMod * staminaMod;
                 defenseCount++;
            }
            if (['ap', 'b2b', 'wm'].includes(p.roleId)) {
                attackSum += ((p.attributes.shooting) + p.attributes.dribbling + p.attributes.passing) / 3 * formMoraleMod * staminaMod;
                attackCount++;
            }
        }
        if (role.category === 'DF') {
            defenseSum += ((p.attributes.tackling * 1.2) + p.attributes.positioning + p.attributes.speed) / 3 * formMoraleMod * staminaMod;
            defenseCount++;
        }
        if (role.category === 'GK') {
        ...
        players.forEach(otherPlayer => {
            if (p.id < otherPlayer.id) { // Avoid double counting and self-comparison
                totalChemistry += calculateChemistryScore(p, otherPlayer, chemistryMatrix, relationships, mentoringGroups);
                chemistryLinks++;
            }
        });
    });
    ...
    const defense = (defenseCount > 0 ? defenseSum / defenseCount : 50) * familiarityMod * chemistryMod;
    const midfield = (midfieldCount > 0 ? midfieldSum / midfieldCount : 50) * familiarityMod * chemistryMod;
...
export const simulateMatch = (
    ...
    for (let minute = 1; minute <= 90; minute++) {
        ...
        const homeTacticMods = {
            goalChance: (TACTIC_MODIFIERS.mentality as any)[homeTactics.mentality]?.goalChance ?? 1.0,
            defensiveAction: (TACTIC_MODIFIERS.mentality as any)[homeTactics.mentality]?.defensiveAction ?? 1.0,
            foulChance: (TACTIC_MODIFIERS.pressing as any)[homeTactics.pressing]?.foulChance ?? 1.0,
        };
        const awayTacticMods = {
            goalChance: (TACTIC_MODIFIERS.mentality as any)[awayTactics.mentality]?.goalChance ?? 1.0,
            defensiveAction: (TACTIC_MODIFIERS.mentality as any)[awayTactics.mentality]?.defensiveAction ?? 1.0,
            foulChance: (TACTIC_MODIFIERS.pressing as any)[awayTactics.pressing]?.foulChance ?? 1.0,
        };
        ...
        const chanceCreation = possession === 'home' ? 0.1 * homePossessionAdvantage : 0.1 * (1 - homePossessionAdvantage);
        if (Math.random() < chanceCreation) {
            const [attackers, defenders, strengths, tacticMods] = possession === 'home'
                ? [simHomePlayers, simAwayPlayers, homeStrengths, homeTacticMods]
                : [simAwayPlayers, simHomePlayers, awayStrengths, awayTacticMods];
            const [defendingStrengths, defendingTacticMods] = possession === 'home'
                ? [awayStrengths, awayTacticMods]
                : [homeStrengths, homeTacticMods];
            ...
            if (Math.random() < goalProbability) {
                ...
                if (potentialAssisters.length > 0 && Math.random() < 0.7) {
                    const assistPlayer = potentialAssisters[Math.floor(Math.random() * potentialAssisters.length)];
                    assister = assistPlayer || null;
                }
                const goalCommentary = { minute, text: assister ? `GOAL! ${scorer.name} finishes it off, assisted by ${assister.name}!` : `GOAL! A brilliant solo effort from ${scorer.name}!` };
        ...
        allPlayers.forEach(player => {
            ...
            if (Math.random() < playerFoulChance) {
                 ...
                 commentaryLog.push(commentary);
                 onUpdate(commentary);
                 const event: MatchEvent = { minute, type: 'Yellow Card', team: player.team, playerName: player.name, description: 'received a yellow card for a late challenge.' };
                 events.push(event);
                 onUpdate(event);
            }
        });
    ...
    return { homeScore, awayScore, events, commentaryLog, isRivalry: Math.random() < 0.2, playerStats };
};
...
export const generateMatchPreview = (
    ...
    const homeKeyPlayers = homePlayers
        .sort((a, b) => {
            ...
            const bOverall = (b.attributes.shooting + b.attributes.passing + b.attributes.tackling + b.attributes.positioning) / 4;
