...
function calculatePlayerSlotScore(player: Player, slot: FormationSlot): number {
    ...
    if (playerRole) {
      switch (slotCategory) {
        case 'GK':
          ...
          break;
        case 'DF':
          score += playerRole.category === 'DF' ? 70 : playerRole.category === 'MF' ? 30 : 0;
          break;
        case 'MF':
          score +=
            playerRole.category === 'MF'
              ? 70
              : playerRole.category === 'DF'
                ? 40
                : playerRole.category === 'FW'
                  ? 40
                  : 0;
          break;
        case 'FW':
          score += playerRole.category === 'FW' ? 70 : playerRole.category === 'MF' ? 30 : 0;
          break;
      }
    }
  }
  ...
  if (slot.role === 'GK') {
    ...
    score +=
      (attributes.tackling * 0.4 + attributes.positioning * 0.3 + attributes.speed * 0.2) * 0.5;
    ...
    score +=
      (attributes.shooting * 0.4 + attributes.speed * 0.3 + attributes.dribbling * 0.3) * 0.5;
  }
...
export function autoAssignPlayersToFormation(
  ...
  for (const slot of unassignedSlots) {
    if (unassignedPlayers.length > 0) {
      ...
      (slot as any).playerId = bestPlayer.id;
...
export function smartPlayerSwap(
  ...
  if (availableSlots.length > 0) {
    ...
    if (bestAlternative.score > 40) {
      // Only suggest if reasonably good fit
      recommendations.push({
...
export function getFormationAnalysis(
  ...
  const formationMetrics = {
    defensiveStrength:
      roleStrengths.DF / Math.max(1, formation.slots.filter(s => s.role === 'DF').length),
    midfielderControl:
      roleStrengths.MF / Math.max(1, formation.slots.filter(s => s.role === 'MF').length),
    attackingThreat:
      roleStrengths.FW / Math.max(1, formation.slots.filter(s => s.role === 'FW').length),
