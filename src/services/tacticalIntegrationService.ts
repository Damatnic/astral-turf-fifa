import { type Formation, type Player, type MatchEvent } from '../types';
...
class TacticalIntegrationService {
  ...
  private getFallbackAnalysis(formation: Formation, players: Player[]): TacticalAnalysis {
  ...
  private identifyKeyBattles(home: Formation, away: Formation): string[] {
    return [
      'Midfield control',
  ...
  private calculatePositionMatch(player: Player, formation: Formation): number {
    // Simplified position matching - would need more complex logic
    return Math.random() * 40 + 40; // 40-80 base chemistry
  }

  private calculateNeighborBonus(player: Player, players: Player[], formation: Formation): number {
  ...
  private calculateZoneIntensity(player: Player, formation: Formation): number {
