...
import type { Player, RootState, MentoringGroup, PlayerRelationshipType, Formation, TacticsState } from '../types';
        ...
        onFieldPlayers.forEach(playerB => {
            if (playerA.id !== playerB.id) {
                ...
                if (relationship === 'rivalry') {
                    rivalries.push(`${playerA.name} and ${playerB.name} have a rivalry that may affect team chemistry.`);
                }
            }
        });
    });
