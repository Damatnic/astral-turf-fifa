...
class GeometricAnalyzer {
  ...
  private analyzeZonalPatterns(zones: DrawingShape[]): GeometricPattern[] {
    ...
    zones.forEach(zone => {
      ...
      if (center.y < 30) { // Defensive third
        patterns.push({
          type: 'defensive_zone',
          shape: zone,
          confidence: 0.9,
      ...
      } else if (center.y > 70) { // Attacking third
        patterns.push({
          type: 'attacking_zone',
          shape: zone,
          confidence: 0.9,
  ...
  private analyzeMovementPatterns(arrows: DrawingShape[]): GeometricPattern[] {
    ...
    arrows.forEach(arrow => {
      ...
      if (direction.y > 0.7 && length > 20) { // Forward movement
        patterns.push({
          type: 'attacking_run',
          shape: arrow,
          confidence: 0.8,
      ...
      } else if (direction.y < -0.5 && length > 15) { // Backward movement
        patterns.push({
      ...
      } else if (Math.abs(direction.x) > 0.8 && length > 20) { // Lateral movement
        patterns.push({
          type: 'width_run',
          shape: arrow,
          confidence: 0.8,
  ...
  private calculateLineLength(points: { x: number; y: number }[]): number {
  ...
  private calculateLineAngle(points: { x: number; y: number }[]): number {
    if (points.length < 2) return 0;
  ...
  private calculateZoneCenter(points: { x: number; y: number }[]): { x: number; y: number } {
  ...
  private calculateZoneArea(points: { x: number; y: number }[]): number {
  ...
  private calculateArrowDirection(points: { x: number; y: number }[]): { x: number; y: number } {
  ...
  private isRotationalMovement(points: { x: number; y: number }[]): boolean {
...
class TacticalPatternRecognizer {
  ...
  private recognizeFormationPatterns(players: Player[], formation: Formation): RecognizedPattern[] {
    ...
    if (width > 60) {
      patterns.push({
        ...
        implications: ['Stretches opposition', 'Creates space centrally', 'May be vulnerable to quick passing']
      });
    }
    ...
    if (depth > 70) {
      patterns.push({
        ...
        implications: ['Good defensive coverage', 'Multiple passing layers', 'May struggle with high press']
      });
    }
  ...
  private recognizeDrawingPatterns(shapes: DrawingShape[]): RecognizedPattern[] {
    ...
    if (this.hasTriangularPattern(lines)) {
      patterns.push({
        ...
        implications: ['High possession retention', 'Patient build-up play', 'Requires technical players']
      });
    }
    ...
    if (this.hasOverloadPattern(arrows, zones)) {
      patterns.push({
        ...
        implications: ['Creates advantage in key areas', 'May leave other areas exposed', 'Requires coordination']
      });
    }
    ...
    if (this.hasPressingPattern(arrows, zones)) {
      patterns.push({
        ...
        implications: ['Forces opposition errors', 'Requires high stamina', 'Risk if press is broken']
      });
    }
  ...
  private recognizeCombinedPatterns(shapes: DrawingShape[], players: Player[]): RecognizedPattern[] {
    const patterns: RecognizedPattern[] = [];
    ...
    if (arrows.length > 2 && fastPlayers.length > 3) {
      ...
      if (forwardArrows.length > 1) {
        patterns.push({
          ...
          implications: ['Effective against high defensive lines', 'Requires quick decision making', 'Vulnerable when caught forward']
        });
      }
    }
    ...
    if (technicalPlayers.length > 3 && centralZones.length > 0) {
      patterns.push({
        ...
        implications: ['High possession percentage', 'Patient build-up play', 'May struggle against intense pressing']
      });
    }
  ...
  private suggestImprovements(patterns: RecognizedPattern[], shapes: DrawingShape[]): Improvement[] {
    ...
    if (patterns.some(p => p.name === 'Counter-Attack Setup') && 
  ...
  private hasTriangularPattern(lines: DrawingShape[]): boolean {
    ...
    return lines.length >= 3 && lines.some(line => 
      this.calculateLineLength(line.points) < 25
    );
  }
  ...
  private getCompactPassingLanes(shapes: DrawingShape[]): { x: number; y: number }[] {
    ...
    if (existingPositions.length < 2) return [{ x: 50, y: 50 }];
...
class SmartAnnotationEngine {
  ...
  private findNearestPlayer(position: { x: number; y: number }, players: Player[]): Player | null {
    ...
    players.forEach(player => {
      ...
      if (distance < minDistance && distance < 20) { // Within reasonable range
  ...
  private calculateArrowCenter(points: { x: number; y: number }[]): { x: number; y: number } {
    if (points.length === 0) return { x: 0, y: 0 };
  ...
  private calculateLineCenter(points: { x: number; y: number }[]): { x: number; y: number } {
...
export class AIDrawingIntelligenceService {
  ...
  async analyzeDrawings(
    ...
    const tacticalRecognition = this.tacticalPatternRecognizer.recognizePatterns(shapes, players, formation);
  ...
  private calculateOverallAnalysisScore(
    ...
    const geometricScore = geometricPatterns.reduce((sum, p) => sum + p.confidence, 0) / geometricPatterns.length || 0;
  ...
  private generateOverallRecommendations(
    ...
    if (tacticalRecognition.improvements.length > 0) {
      recommendations.push(`Address ${tacticalRecognition.improvements.length} identified improvements`);
    }
      ...
      case 'attacking':
        if (!shapes.some(s => s.tool === 'arrow')) {
          missing.push({
            ...
            priority: 'high',
            positions: [{ x: 50, y: 70 }, { x: 30, y: 75 }, { x: 70, y: 75 }],
      ...
      case 'defensive':
        if (!shapes.some(s => s.tool === 'zone')) {
          missing.push({
            ...
            priority: 'high',
            positions: [{ x: 50, y: 30 }, { x: 30, y: 25 }, { x: 70, y: 25 }],
  ...
  private findMissingCriticalElements(
    ...
    const hasDefensiveElements = shapes.some(s => 
      s.tool === 'zone' && this.calculateZoneCenter(s.points).y < 50
    );
...
export interface SmartAnnotation {
  ...
  text: string;
  type: 'zone_description' | 'movement_description' | 'passing_description' | 'player_instruction' | 'pattern_explanation' | 'sequence_number';
  priority: 'low' | 'medium' | 'high';
}
