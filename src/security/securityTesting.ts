...
export class GuardianSecurityTesting {
      ...
      // Authentication Tests
      {
        ...
        test: async (): Promise<TestResult> => {
          ...
          for (const password of weakPasswords) {
            try {
              const validation = validateAndSanitize({ password }, {
          ...
          return {
            ...
            message: results.length === 0 ? 'Password validation working correctly' : `Weak passwords accepted: ${results.length}`,
            details: results,
        ...
        test: async (): Promise<TestResult> => {
          ...
          for (const payload of sqlPayloads) {
            try {
              ...
              if (JSON.stringify(result).includes('DROP') || 
          ...
          return {
            ...
            message: vulnerabilities.length === 0 ? 'SQL injection protection working' : `${vulnerabilities.length} vulnerabilities found`,
            details: vulnerabilities,
            recommendations: vulnerabilities.length > 0 ? ['Improve input sanitization', 'Use parameterized queries'] : [],
            timestamp: new Date().toISOString(),
            executionTime: 0
          };
        }
      ...
      // XSS Prevention Test
      {
        ...
        test: async (): Promise<TestResult> => {
          ...
          return {
            testId: 'input_002',
            passed: vulnerabilities.length === 0,
            message: vulnerabilities.length === 0 ? 'XSS protection working' : `${vulnerabilities.length} vulnerabilities found`,
            details: vulnerabilities,
            recommendations: vulnerabilities.length > 0 ? ['Improve HTML encoding', 'Use Content Security Policy'] : [],
            timestamp: new Date().toISOString(),
            executionTime: 0
          };
        }
      ...
      // Data Encryption Test
      {
        ...
        test: async (): Promise<TestResult> => {
          ...
          try {
            ...
            issues.push(`Encryption/decryption error: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }

          return {
            testId: 'crypto_001',
            passed: issues.length === 0,
            message: issues.length === 0 ? 'Encryption working correctly' : `${issues.length} encryption issues found`,
            details: issues,
            recommendations: issues.length > 0 ? ['Fix encryption implementation', 'Verify key management'] : [],
            timestamp: new Date().toISOString(),
            executionTime: 0
          };
        }
        ...
        test: async (): Promise<TestResult> => {
          ...
          for (const fileData of maliciousFiles) {
            try {
              ...
              const result = await guardianSecureFileHandler.secureImport(file, 'test-user', 'test-team');
          ...
          return {
            testId: 'file_001',
            passed: issues.length === 0,
            message: issues.length === 0 ? 'File upload security working' : `${issues.length} file security issues`,
            details: issues,
            recommendations: issues.length > 0 ? ['Strengthen file validation', 'Implement virus scanning'] : [],
            timestamp: new Date().toISOString(),
            executionTime: 0
          };
        }
      ...
      // Threat Detection Test
      {
        ...
        test: async (): Promise<TestResult> => {
          ...
          try {
            ...
            const hasSQLInjectionThreat = threats.some(t => t.threatType === ThreatType.SQL_INJECTION);
            if (!hasSQLInjectionThreat) {
              issues.push('SQL injection threat not properly classified');
            }

          } catch (error) {
            issues.push(`Threat detection error: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }

          return {
            testId: 'threat_001',
            passed: issues.length === 0,
            message: issues.length === 0 ? 'Threat detection working' : `${issues.length} threat detection issues`,
            details: issues,
            recommendations: issues.length > 0 ? ['Improve threat detection rules', 'Update threat signatures'] : [],
            timestamp: new Date().toISOString(),
            executionTime: 0
          };
        }
      ...
      // Tactical Data Validation Test
      {
        ...
        test: async (): Promise<TestResult> => {
          ...
          for (const formation of invalidFormations) {
            ...
            if (result.valid) {
              issues.push(`Invalid formation data accepted: ${JSON.stringify(formation).substring(0, 100)}`);
            }
          }
          ...
          return {
            testId: 'tactical_001',
            passed: issues.length === 0,
            message: issues.length === 0 ? 'Tactical data validation working' : `${issues.length} validation issues`,
            details: issues,
    ...
    const penetrationTests: PenetrationTestScenario[] = [
      {
        ...
        test: async (): Promise<PenetrationTestResult> => {
          ...
          return {
            ...
            defensesTriggered,
            recommendations: vulnerabilities.length > 0 ? ['Implement stronger authentication defenses'] : [],
            executionTime: Date.now() - startTime,
            timestamp: new Date().toISOString()
          };
        }
  ...
  private calculateAssessment(results: TestResult[]): VulnerabilityAssessment {
    ...
    const criticalIssues = results.filter(r => !r.passed && 
      this.securityTests.get(r.testId)?.severity === TestSeverity.CRITICAL).length;
    const highIssues = results.filter(r => !r.passed && 
      this.securityTests.get(r.testId)?.severity === TestSeverity.HIGH).length;
    const mediumIssues = results.filter(r => !r.passed && 
      this.securityTests.get(r.testId)?.severity === TestSeverity.MEDIUM).length;
    const lowIssues = results.filter(r => !r.passed && 
      this.securityTests.get(r.testId)?.severity === TestSeverity.LOW).length;
