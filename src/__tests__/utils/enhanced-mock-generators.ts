...
export const generateFormationSlot = (overrides: Partial<FormationSlot> = {}): FormationSlot => {
  ...
  const positionsByRole = {
    ...
    DF: { x: faker.number.float({ min: 20, max: 35 }), y: faker.number.float({ min: 20, max: 80 }) },
    MF: { x: faker.number.float({ min: 40, max: 70 }), y: faker.number.float({ min: 25, max: 75 }) },
    FW: { x: faker.number.float({ min: 75, max: 90 }), y: faker.number.float({ min: 30, max: 70 }) }
  };
...
export const generateTacticalFormation = (
  ...
  const formationConfigs = {
    '4-4-2': [
      ...
      { role: 'DF' as const, count: 4, positions: [
        { x: 25, y: 20 }, { x: 25, y: 40 }, { x: 25, y: 60 }, { x: 25, y: 80 }
      ]},
      { role: 'MF' as const, count: 4, positions: [
        { x: 50, y: 25 }, { x: 50, y: 45 }, { x: 50, y: 55 }, { x: 50, y: 75 }
      ]},
      { role: 'FW' as const, count: 2, positions: [
        { x: 80, y: 40 }, { x: 80, y: 60 }
      ]}
    ],
    '4-3-3': [
      ...
      { role: 'MF' as const, count: 3, positions: [
        { x: 50, y: 35 }, { x: 50, y: 50 }, { x: 50, y: 65 }
      ]},
      { role: 'FW' as const, count: 3, positions: [
        { x: 80, y: 30 }, { x: 80, y: 50 }, { x: 80, y: 70 }
      ]}
    ],
    '3-5-2': [
      { role: 'GK' as const, count: 1, positions: [{ x: 10, y: 50 }] },
      { role: 'DF' as const, count: 3, positions: [
        { x: 25, y: 30 }, { x: 25, y: 50 }, { x: 25, y: 70 }
      ]},
      { role: 'MF' as const, count: 5, positions: [
        { x: 35, y: 20 }, { x: 35, y: 80 }, { x: 50, y: 35 }, { x: 50, y: 50 }, { x: 50, y: 65 }
      ]},
      { role: 'FW' as const, count: 2, positions: [
        { x: 80, y: 40 }, { x: 80, y: 60 }
      ]}
    ...
    '5-3-2': [
      { role: 'GK' as const, count: 1, positions: [{ x: 10, y: 50 }] },
      { role: 'DF' as const, count: 5, positions: [
        { x: 20, y: 15 }, { x: 25, y: 35 }, { x: 25, y: 50 }, { x: 25, y: 65 }, { x: 20, y: 85 }
      ]},
      { role: 'MF' as const, count: 3, positions: [
        { x: 50, y: 35 }, { x: 50, y: 50 }, { x: 50, y: 65 }
      ]},
  ...
  for (const roleConfig of config) {
    for (let i = 0; i < roleConfig.count; i++) {
      slots.push(generateFormationSlot({
...
export const generateDrawingShape = (overrides: Partial<DrawingShape> = {}): DrawingShape => {
  ...
  const pointsConfigs = {
    ...
    zone: () => Array.from({ length: 4 }, () => ({
      x: faker.number.float({ min: 20, max: 80 }),
      y: faker.number.float({ min: 20, max: 80 })
    })),
    pen: () => Array.from({ length: faker.number.int({ min: 5, max: 20 }) }, () => ({
...
function generatePlayerForConflict(role: PositionRole): Player {
  const basePlayer = {
    ...
    potential: [faker.number.int({ min: 70, max: 85 }), faker.number.int({ min: 85, max: 95 })] as const,
 ...
 */
export const generateChemistryData = (players: Player[]): Array<{
  player1Id: string;
  player2Id: string;
      ...
      const factors = [
        {
          ...
          description: player1.nationality === player2.nationality 
            ? 'Same nationality' : 'Different nationalities'
        },
        {
        ...
        },
        {
          ...
          description: Math.abs(player1.age - player2.age) <= 5 
            ? 'Similar ages' : 'Age gap'
        },
        {
...
export const generatePerformanceTestData = {
  ...
  medium: () => ({
    players: Array.from({ length: 25 }, () => generatePlayerForConflict(
      faker.helpers.arrayElement(['GK', 'DF', 'MF', 'FW'])
    )),
  ...
  large: () => ({
    players: Array.from({ length: 100 }, () => generatePlayerForConflict(
      faker.helpers.arrayElement(['GK', 'DF', 'MF', 'FW'])
    )),
...
export const generateEdgeCases = {
  emptyFormation: () => generateTacticalFormation('4-4-2', {
    slots: []
  }),
