 ...
 */
export const simulateDragAndDrop = async (scenario: DragDropTestScenario): Promise<{
  success: boolean;
...
export const testSlotDropZones = async (
  ...
  for (const slot of slots) {
    ...
    // Test drag over
    const dragOverEvent = createDragEvent('dragover', createDataTransferMock({
      'application/json': JSON.stringify({
    ...
    const highlightsOnHover = slotElement.classList.contains('drop-zone-active') ||
    ...
    const positionCorrect = Math.abs(rect.left - expectedX) < 10 && 
                           Math.abs(rect.top - expectedY) < 10;
 ...
 */
export const testComplexDragScenario = async (scenarios: Array<{
  players: Player[];
  ...
  for (const [index, scenario] of scenarios.entries()) {
    ...
    try {
      ...
      for (const [playerIndex, player] of scenario.players.entries()) {
        ...
        const playerElement = document.querySelector(`[data-player-id="${player.id}"]`) as HTMLElement;
        const slotElement = document.querySelector(`[data-slot-id="${targetSlot.id}"]`) as HTMLElement;
 ...
 */
export const waitForDragAnimation = async (element: HTMLElement, maxWait: number = 1000): Promise<boolean> => {
