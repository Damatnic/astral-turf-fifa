...
vi.mock('react-chartjs-2', () => ({
  Line: ({ data }: any) => (
    <div data-testid="line-chart">Line Chart: {(data as any).labels?.join(', ')}</div>
  ),
  Bar: ({ data }: any) => (
    <div data-testid="bar-chart">Bar Chart: {(data as any).labels?.join(', ')}</div>
  ),
  Doughnut: ({ data }: any) => (
    <div data-testid="doughnut-chart">Doughnut Chart: {(data as any).labels?.join(', ')}</div>
  ),
  Radar: ({ data }: any) => (
    <div data-testid="radar-chart">Radar Chart: {(data as any).labels?.join(', ')}</div>
  ),
}));
