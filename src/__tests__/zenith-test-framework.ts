...
export class ZenithComponentTester {
  ...
  private hasDescription(): boolean {
    return !!this.defaultProps['aria-describedby'] || 
  ...
  private generateErrorProps(): any {
    return {
      ...this.defaultProps,
      onError: () => { throw new Error('Test error'); },
      children: null,
    };
  }
}
...
export const ZenithTestUtils = {
  ...
  checkAccessibility: async (container: HTMLElement): Promise<boolean> => {
    ...
    const interactiveElements = container.querySelectorAll('button, input, select, textarea, a, [tabindex]');
