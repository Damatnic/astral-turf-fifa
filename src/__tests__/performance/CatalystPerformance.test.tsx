...
import { formationCache, playerCache, queryCache } from '../../utils/cachingOptimizations';
import { AppProvider } from '../../context/AppProvider';

// Aliases for cache instances used in tests
const globalMemoryCache = queryCache;
const globalMultiLayerCache = formationCache;
const cachedHTTPClient = playerCache;

// Mock performance APIs
const mockPerformanceObserver = vi.fn() as any;
...
describe('Catalyst Performance Test Suite', () => {
  ...
  describe('Caching Performance', () => {
    it('should achieve high cache hit rates', async () => {
      ...
      // Set cache entries
      for (let i = 0; i < 100; i++) {
        await (globalMultiLayerCache as any).set(`test-key-${i}`, { ...testData, id: i });
      }
    ...
    it('should handle cache invalidation efficiently', async () => {
      // Populate cache
      for (let i = 0; i < 50; i++) {
        await (globalMultiLayerCache as any).set(`invalidation-test-${i}`, { data: i });
      }
  ...
  describe('Network Performance', () => {
    ...
    it('should efficiently handle concurrent API requests', async () => {
      ...
      const requests = Array.from({ length: 10 }, (_, i) => cachedHTTPClient.get(`/api/test-${i}`));
