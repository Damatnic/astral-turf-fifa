...
import { generateChemistryData, generatePerformanceTestData } from '../utils/enhanced-mock-generators';
...
describe('ChemistryVisualization Performance Tests', () => {
  ...
  const endPerformanceMonitoring = (playerCount: number): ChemistryCalculationMetrics => {
    ...
    return {
      ...
      cacheHitRate: performanceMonitor.cacheHits / 
  ...
  const calculateAttributeCompatibility = (
    ...
    const attributes = ['speed', 'passing', 'tackling', 'shooting', 'dribbling', 'positioning'] as const;
  ...
  const calculateTeamChemistry = (players: Player[]): Array<{
    player1Id: string;
    player2Id: string;
    chemistry: number;
      ...
      for (let j = i + 1; j < players.length; j++) {
        ...
        chemistryPairs.push({
          ...
          factors: [
            ...
            { factor: 'position', impact: calculatePositionSynergy(player1.roleId, player2.roleId) },
  ...
  describe('Small Dataset Performance', () => {
    it('should calculate chemistry for 11 players efficiently', () => {
      ...
      expect(metrics.calculationTime / metrics.playerPairsProcessed)
        .toBeLessThan(PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair);
  ...
  describe('Medium Dataset Performance', () => {
    it('should handle squad-sized calculations efficiently', () => {
      ...
      expect(metrics.calculationTime).toBeLessThan(PERFORMANCE_THRESHOLDS.maxTotalCalculationTime * 5);
      expect(metrics.calculationTime / metrics.playerPairsProcessed)
        .toBeLessThan(PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair * 2);
  ...
  describe('Large Dataset Performance', () => {
    it('should handle large player pools with acceptable performance', () => {
      ...
      expect(metrics.calculationTime / metrics.playerPairsProcessed)
        .toBeLessThan(PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair * 5);
  ...
  describe('Algorithm Optimization', () => {
    ...
    it('should scale efficiently with player count', () => {
      ...
      playerCounts.forEach(count => {
        ...
        const withinThresholds = performanceScore < PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair * 2;
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle extreme attribute values', () => {
      ...
      players[1].attributes = {
        speed: 100, passing: 100, tackling: 100, shooting: 100, dribbling: 100, positioning: 100, stamina: 100
      };
  ...
  describe('Comprehensive Performance Report', () => {
    it('should generate complete performance analysis', () => {
      ...
      testScenarios.forEach(scenario => {
        ...
        const performanceScore = metrics.calculationTime / metrics.playerPairsProcessed;
        const withinThresholds = performanceScore < PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair * 3;
      ...
      const summary = {
        ...
        averagePerformanceScore: performanceReport.reduce((sum, r) => sum + r.performanceScore, 0) / performanceReport.length,
        ...
        scalabilityFactor: performanceReport[performanceReport.length - 1].performanceScore / performanceReport[0].performanceScore
      };

      // Validate overall performance
      expect(summary.passedTests).toBeGreaterThanOrEqual(summary.totalTests * 0.75); // 75% pass rate
      expect(summary.averagePerformanceScore).toBeLessThan(PERFORMANCE_THRESHOLDS.maxCalculationTimePerPair * 2);
