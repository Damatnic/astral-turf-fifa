...
import { autoAssignPlayersToFormation, getFormationAnalysis } from '../../services/formationAutoAssignment';
...
describe('Catalyst Performance Benchmarks', () => {
  describe('Formation Auto-Assignment Performance', () => {
    ...
    it('should maintain O(n log n) complexity scaling', () => {
      ...
      for (const size of sizes) {
        ...
        const startTime = performance.now();
        autoAssignPlayersToFormation(players, formation, 'home');
        const endTime = performance.now();
      ...
      console.log(`✅ Complexity scaling test: ${sizes.map((size, i) => `${size}: ${times[i].toFixed(2)}ms`).join(', ')}`);
    });
  });
  ...
  describe('Formation Analysis Performance', () => {
    it('should analyze formations with 50 players in under 20ms', () => {
      ...
      const startTime = performance.now();
      const result = getFormationAnalysis(formation, players);
      const endTime = performance.now();
      const duration = endTime - startTime;
  ...
  describe('Caching Performance', () => {
    it('should cache and retrieve 1000 items in under 20ms', () => {
      const cache = new UltraFastCache(1000);
      ...
      console.log(`✅ Cache operations: store ${cacheDuration.toFixed(2)}ms, retrieve ${retrieveDuration.toFixed(2)}ms`);
    });
    
    it('should handle cache eviction efficiently', () => {
      const cache = new UltraFastCache(100); // Small cache
  ...
  describe('Memory Efficiency', () => {
    it('should not create memory leaks in repeated operations', () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        const formation = generateMockFormation(11);
        autoAssignPlayersToFormation(players, formation, 'home');
        getFormationAnalysis(formation, players);
      }
  ...
  describe('Bundle Size Validation', () => {
    it('should maintain reasonable component sizes', () => {
      ...
      console.log(`✅ Bundle sizes: ${Object.entries(componentSizes).map(([name, size]) => 
        `${name}: ${(size / 1024).toFixed(1)}KB`).join(', ')}`);
