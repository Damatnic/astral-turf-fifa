  ...
  switch ((action as any).type) {
    case 'LOAD_STATE':
      return {
        ...
        ui: { ...INITIAL_STATE.ui, ...(action as any).payload.ui },
      };
    case 'RESET_STATE':
        ...
        switch ((action as any).type) {
          ...
          case 'ADVANCE_WEEK': {
            ...
            if (ambitiousOrTemperamentalPlayers.length > 1) {
              for (let i = 0; i < ambitiousOrTemperamentalPlayers.length; i++) {
                for (let j = i + 1; j < ambitiousOrTemperamentalPlayers.length; j++) {
                  ...
                  if (Math.random() < 0.02) {
                    // 2% chance per week
                    if (!draft.franchise.relationships[p1.id]) {
                      draft.franchise.relationships[p1.id] = {};
                    }
            ...
            teamsToProcess.forEach(team => {
              ...
              const fitnessCoachBonus = teamStaff.fitnessCoach
                ? (teamStaff.fitnessCoach.fitnessCoaching / 20) * 0.2
                : 0; // Max 20% bonus
              const assistantManagerBonus = teamStaff.assistantManager
                ? (teamStaff.assistantManager.tacticalKnowledge / 20) * 0.25
                : 0; // Max 25% bonus
              ...
              (Object.values(teamSchedule) as DailySchedule[]).forEach(day => {
                ...
                const sessionDrills = [
                  day.morning.warmup,
                  day.morning.main,
                  day.morning.cooldown,
                  day.afternoon.warmup,
                  day.afternoon.main,
                  day.afternoon.cooldown,
                ];
              ...
              if (activeFormationId && tacticalDrillCount > 0) {
                const familiarityIncrease = tacticalDrillCount * 0.5 * (1 + assistantManagerBonus);
                  ...
                  if (day.isRestDay) {
                    ...
                    dailyDrills.forEach(drillId => {
                      ...
                      const processAttribute = (
                        ...
                        if (player.currentPotential < player.potential[1]) {
                          points *= 1.5;
                        }
                        ...
                        if (newProgress >= DEVELOPMENT_THRESHOLD) {
                          ...
                          player.attributeDevelopmentProgress[attr] =
                            newProgress - DEVELOPMENT_THRESHOLD;
          ...
          case 'SIMULATE_MATCH_UPDATE': {
            draft.ui.simulationTimeline.push((action as any).payload);
            break;
          }
          ...
          case 'SIMULATE_MATCH_SUCCESS': {
            ...
            result.events.forEach(event => {
              if (event.type === 'Goal' && event.assisterName) {
                ...
                if (scorer && assister && Math.random() < 0.25) {
                  ...
                  if (!draft.franchise.relationships[scorer.id]) {
                    draft.franchise.relationships[scorer.id] = {};
                  }
            ...
            const userTeamName =
              draft.franchise.season.leagueTeams.find(t => t.isUser)?.name || 'Astral FC';
          ...
          case 'ADD_LIBRARY_PLAY_TO_PLAYBOOK': {
            ...
            draft.tactics.playbook[newId] = { ...(action as any).payload, id: newId };
            // Now, set the state to load this new item
            const { formationId, steps } = (action as any).payload;
          ...
          case 'CREATE_PLAYBOOK_ITEM': {
            const { name, category } = (action as any).payload;
            ...
            const initialStep: PlaybookStep = {
              id: 'step1',
              playerPositions: draft.tactics.players.reduce(
                (acc, player) => {
          ...
          case 'SET_PLAYBOOK_EVENT': {
            const activeItemId = draft.ui.activePlaybookItemId;
            const { stepIndex, event } = (action as any).payload;
