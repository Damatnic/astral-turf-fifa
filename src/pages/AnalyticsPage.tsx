...
const AnalyticsPage: React.FC = () => {
    ...
    const [selectedMetric, setSelectedMetric] = useState<'performance' | 'financials' | 'transfers' | 'development'>('performance');
    ...
    const calculatePerformanceMetrics = () => {
        ...
        const goalStats = matchHistory.reduce((acc, match) => {
    ...
    const calculatePlayerAnalytics = () => {
        ...
        const avgPotential = teamPlayers.reduce((sum, p) => sum + p.currentPotential, 0) / teamPlayers.length || 0;
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">
                                            {performanceMetrics.wins}W {performanceMetrics.draws}D {performanceMetrics.losses}L
                                        </p>
                                    </div>
                            ...
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-blue-400">{performanceMetrics.goalsScored}</p>
                                        <p className="text-xs text-gray-500">
                                            {(performanceMetrics.goalsScored / Math.max(performanceMetrics.totalMatches, 1)).toFixed(1)} per game
                                        </p>
                                    </div>
                            ...
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-red-400">{performanceMetrics.goalsConceded}</p>
                                        <p className="text-xs text-gray-500">
                                            {(performanceMetrics.goalsConceded / Math.max(performanceMetrics.totalMatches, 1)).toFixed(1)} per game
                                        </p>
                                    </div>
                            ...
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className={`text-2xl font-bold ${performanceMetrics.goalDifference >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                            {performanceMetrics.goalDifference >= 0 ? '+' : ''}{performanceMetrics.goalDifference}
                                        </p>
                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                ...
                                <p className="text-2xl font-bold text-white">
                                    ${(teamFinances.income.ticketSales + teamFinances.income.sponsorship + teamFinances.income.prizeMoney).toLocaleString()}
                                </p>
                            </div>
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                            ...
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                ...
                                <p className="text-2xl font-bold text-white">${playerAnalytics.squadValue.toLocaleString()}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="bg-gray-700 rounded-lg p-4 text-center">
                                    ...
                                    <p className="text-xl font-bold text-green-400">${teamFinances.income.ticketSales.toLocaleString()}</p>
                                </div>
                                <div className="bg-gray-700 rounded-lg p-4 text-center">
                                    <p className="text-sm text-gray-400">Sponsorship</p>
                                    <p className="text-xl font-bold text-green-400">${teamFinances.income.sponsorship.toLocaleString()}</p>
                                </div>
                                <div className="bg-gray-700 rounded-lg p-4 text-center">
                                    <p className="text-sm text-gray-400">Prize Money</p>
                                    <p className="text-xl font-bold text-green-400">${teamFinances.income.prizeMoney.toLocaleString()}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                {teamPlayers.slice(0, 9).map((player) => (
                                    <div key={player.id} className="bg-gray-700 rounded-lg p-4">
                                        ...
                                        <div className="space-y-1">
                                            {Object.entries(player.attributeDevelopmentProgress).slice(0, 2).map(([attr, progress]) => (
                ...
                {selectedMetric === 'transfers' && (
                    <div className="space-y-6">
                        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 text-center">
                            ...
                            <p className="text-gray-500">Transfer analytics will be available once you complete some transfers</p>
                        </div>
                    </div>
                )}
            </div>
