...
const NewsFeedPage: React.FC = () => {
    ...
    const getNewsTypeBackground = (type: NewsItem['type']) => {
        switch (type) {
            case 'transfer': return 'bg-blue-600/20';
            case 'injury': return 'bg-red-600/20';
            case 'result': return 'bg-green-600/20';
            case 'rumor': return 'bg-yellow-600/20';
            case 'social_media': return 'bg-purple-600/20';
    ...
    const newsTypes = [
        ...
        { key: 'result' as const, label: 'Match Results', count: newsFeed.filter(n => n.type === 'result').length },
        { key: 'transfer' as const, label: 'Transfers', count: newsFeed.filter(n => n.type === 'transfer').length },
        { key: 'injury' as const, label: 'Injuries', count: newsFeed.filter(n => n.type === 'injury').length },
        { key: 'rumor' as const, label: 'Rumors', count: newsFeed.filter(n => n.type === 'rumor').length },
        { key: 'social_media' as const, label: 'Social Media', count: newsFeed.filter(n => n.type === 'social_media').length },
    ];
        ...
        <div className="w-full h-full p-6 bg-gray-900 overflow-y-auto">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-gray-400">Stay updated with the latest football news and developments</p>
                </div>
                ...
                <div className="bg-gray-800 rounded-lg p-4 mb-6 border border-gray-700">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <p className="text-white font-medium">Season {season.year} - Week {gameWeek}</p>
                                                ...
                                                <div className="flex items-start justify-between mb-2">
                                                    <div>
                                                        ...
                                                        <div className="flex items-center space-x-3">
                                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getNewsTypeBackground(newsItem.type)} ${getNewsTypeColor(newsItem.type)} capitalize`}>
                                                ...
                                                {newsItem.type === 'transfer' && (
                                                    <div className="mt-4 p-3 bg-gray-700/50 rounded-lg">
                                                        <div className="flex items-center space-x-4">
                                                            ...
                                                            <span className="text-sm text-gray-300">Transfer Market Activity</span>
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {newsItem.type === 'rumor' && (
                                                    <div className="mt-4 p-3 bg-yellow-600/10 rounded-lg border border-yellow-600/30">
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <span className="text-xs text-yellow-400 font-medium">Unconfirmed Report</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        {newsTypes.slice(1).map(({ key, label, count }) => (
                            <div key={key} className="text-center">
                                <div className={`text-2xl font-bold ${getNewsTypeColor(key)}`}>
                                    {count}
                                </div>
