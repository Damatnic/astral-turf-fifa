...
const MyPlayerRankingPage: React.FC = () => {
  ...
  const { state, dispatch, getActiveChallenges, getRecommendedChallenges, spendAttributePoints } = useChallengeContext();
  ...
  const handleAttributeSpend = (attribute: keyof typeof selectedPlayer.attributes, points: number) => {
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
          <div className="flex items-start justify-between">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-400">{profile.totalStats.totalChallengesCompleted}</p>
                <p className="text-xs text-gray-500">Challenges</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-green-400">{profile.unspentAttributePoints}</p>
                <p className="text-xs text-gray-500">Unspent Points</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-400">{profile.weeklyProgress.xpEarned}</p>
                <p className="text-xs text-gray-500">Weekly XP</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-yellow-400">{Math.round(profile.totalStats.completionRate)}%</p>
          ...
          <div className="flex space-x-1 mt-6 border-t border-gray-700 pt-4">
            {(['overview', 'attributes', 'badges', 'challenges', 'comparison'] as TabType[]).map(tab => (
              <button
                key={tab}
          ...
          >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                  ...
                  {activeChallenges.length > 0 ? (
                    ...
                    <p className="text-gray-500">No active challenges. Start one to begin earning XP!</p>
                  )}
                  <button
                ...
                <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 lg:col-span-2">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <p className="text-xl font-bold text-white capitalize">{profile.totalStats.favoriteCategory}</p>
                    </div>
                    <div className="bg-gray-700 rounded-lg p-4">
                    ...
                    <div className="bg-gray-700 rounded-lg p-4">
                      ...
                      <p className="text-xl font-bold text-white">{profile.totalStats.difficultyBreakdown.elite || 0}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                  ...
                  <div className="space-y-3">
                    {Object.entries(profile.totalStats.difficultyBreakdown).map(([difficulty, count]) => {
                      const total = Object.values(profile.totalStats.difficultyBreakdown).reduce((a, b) => a + b, 0);
                      ...
                      return (
                        <div key={difficulty}>
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-sm font-medium text-white">{count} completed</span>
                          </div>
                          <div className="w-full bg-gray-700 rounded-full h-2">
                            <div
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {profile.challengesCompleted.slice(-5).reverse().map(challengeId => {
                        const challenge = state.challenges.find(c => c.id === challengeId);
                        if (!challenge) {return null;}
                        return (
                          <div key={challengeId} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                            <div>
                              <p className="text-sm font-medium text-white">{challenge.title}</p>
                              <p className="text-xs text-gray-400">{challenge.xpReward} XP earned</p>
                            </div>
            ...
            {activeTab === 'comparison' && (
              <RankingComparison
                ...
                playerB={comparePlayerId ? tacticsState.players.find(p => p.id === comparePlayerId) : null}
        ...
        {showAttributeSpend && profile.unspentAttributePoints > 0 && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full">
              ...
              <p className="text-gray-400 mb-4">You have {profile.unspentAttributePoints} points to spend</p>
              ...
              <div className="space-y-3">
                {Object.keys(selectedPlayer.attributes).map(attribute => (
                  <div key={attribute} className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => handleAttributeSpend(attribute as keyof typeof selectedPlayer.attributes, 1)}
