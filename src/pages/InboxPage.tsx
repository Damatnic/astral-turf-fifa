...
const InboxPage: React.FC = () => {
    ...
    const [selectedFilter, setSelectedFilter] = useState<'all' | 'unread' | 'match' | 'transfer' | 'injury' | 'contract'>('all');
    ...
    const handleAcceptLoanOffer = (inboxId: string, playerId: string, fee: number, wageContribution: number) => {
    ...
    const getItemTypeColor = (type: InboxItem['type']) => {
        switch (type) {
            ...
            case 'transfer': case 'transfer_offer': return 'text-blue-400';
            case 'injury': return 'text-red-400';
            case 'contract': return 'text-yellow-400';
            case 'award': return 'text-purple-400';
                        ...
                        <div className="bg-gray-800 rounded-lg p-4 mb-6 border border-gray-700">
                            ...
                            <div className="space-y-2">
                                {[
                                    ...
                                    { key: 'match' as const, label: 'Match Results', count: inbox.filter(i => i.type === 'match').length },
                                    { key: 'transfer' as const, label: 'Transfers', count: inbox.filter(i => i.type === 'transfer' || i.type === 'transfer_offer').length },
                                    { key: 'injury' as const, label: 'Injuries', count: inbox.filter(i => i.type === 'injury').length },
                                    { key: 'contract' as const, label: 'Contracts', count: inbox.filter(i => i.type === 'contract').length },
                                ].map(({ key, label, count }) => (
                                    <button
                                        key={key}
                                    ...
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div className="flex-grow min-w-0">
                                            <div className="flex justify-between items-start">
                                                <h4 className={`text-sm font-medium truncate ${!item.isRead ? 'text-white' : 'text-gray-300'}`}>
                                                    {item.title}
                                                </h4>
                        ...
                        {selectedItem ? (
                            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-400">Week {selectedItem.week} • {selectedItem.type}</p>
                                        </div>
                                    </div>
                                    <button
                                ...
                                <div className="bg-gray-700 rounded-lg p-4 mb-6">
                                    <p className="text-gray-200 whitespace-pre-wrap leading-relaxed">{selectedItem.content}</p>
                                </div>
                                    ...
                                    <div className="bg-gray-700 rounded-lg p-4">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex justify-between">
                                                ...
                                                <span className="text-white font-medium">{selectedItem.payload.offer.fromClub}</span>
                                            </div>
                                            <div className="flex justify-between">
                                ...
                                {selectedItem.payload && selectedItem.payload.conversationRequest && (
                                    <div className="bg-gray-700 rounded-lg p-4">
                                        <h3 className="text-lg font-semibold text-teal-400 mb-3">Conversation Request</h3>
