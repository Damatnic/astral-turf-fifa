...
const MedicalCenterPage: React.FC = () => {
    ...
    const getInjuryBackground = (status: string) => {
        switch (status) {
            case 'Minor Injury': return 'bg-yellow-600/10 border-yellow-400';
            case 'Major Injury': return 'bg-red-600/10 border-red-400';
            default: return 'bg-green-600/10 border-green-400';
        }
    };
    ...
    const calculateRecoveryTime = (player: Player) => {
        if (player.availability.status === 'Major Injury') {
            return Math.floor(Math.random() * 8) + 4; // 4-12 weeks
        } else if (player.availability.status === 'Minor Injury') {
            return Math.floor(Math.random() * 3) + 1; // 1-4 weeks
        }
        return 0;
    };
            ...
            <div className="max-w-7xl mx-auto">
                        ...
                        {teamPlayers.map((player) => (
                            <div key={player.id} className="bg-gray-800 rounded-lg p-4 border border-gray-700">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            <span className={`font-bold ${
                                                player.fatigue >= 80 ? 'text-red-400' :
                                                player.fatigue >= 60 ? 'text-yellow-400' : 'text-green-400'
                                            }`}>
                                                {player.fatigue}%
                                            </span>
                                        </div>
                                ...
                                <div className={`mt-4 p-2 rounded text-center text-sm font-medium ${
                                    player.availability.status === 'Available' ? 'bg-green-600/20 text-green-400' :
                                    player.availability.status.includes('Injury') ? 'bg-red-600/20 text-red-400' :
                ...
                {selectedTab === 'prevention' && (
                    <div className="space-y-8">
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-3">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="text-yellow-400">{staff.fitnessCoach.fitnessCoaching}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ) : (
                        ...
                        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                            <h3 className="text-xl font-semibold text-teal-400 mb-4">Prevention Recommendations</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <h4 className="text-lg font-semibold text-white mb-3">Training Load Management</h4>
