...
import { useIntersectionObserver, useLevelOfDetail, useOcclusionCulling } from '../../utils/virtualizationOptimizations';
...
class WebGLRenderer {
  ...
  private initializeWebGL(): void {
    ...
    if (projectionUniform) this.uniforms.set('projection', projectionUniform);
    if (viewUniform) this.uniforms.set('view', viewUniform);
    if (timeUniform) this.uniforms.set('time', timeUniform);
  ...
  addVertex(x: number, y: number, r: number, g: number, b: number, a: number): void {
    if (this.vertexCount >= this.maxVertices) return;
  ...
  render(projectionMatrix: number[], viewMatrix: number[], time: number): void {
    ...
    if (vertexBuffer && colorBuffer) {
      ...
      gl.bufferData(gl.ARRAY_BUFFER, this.vertexData.subarray(0, this.vertexCount * 2), gl.DYNAMIC_DRAW);
...
export const CanvasRenderer: React.FC<CanvasRendererProps> = ({
  ...
  const renderWithWebGL = useCallback((time: number, qualityScale: number) => {
    ...
    for (const batch of renderBatches) {
      if (batch.type === 'player') {
        for (const player of batch.items) {
          ...
          if (lod === 'medium') color.a *= 0.8;
          if (lod === 'low') color.a *= 0.6;
          
          // Scale based on quality
          if (qualityScale < 0.5 && lod !== 'high') continue;
  ...
  const renderWith2D = useCallback((ctx: CanvasRenderingContext2D, time: number, qualityScale: number) => {
    // Clear canvas
    ...
    for (const batch of renderBatches) {
      if (batch.type === 'player') {
        for (const player of batch.items) {
          ...
          if (qualityScale < 0.5 && lod === 'low') continue;
  ...
  const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (const player of players) {
      const distance = Math.sqrt(
        ...
        Math.pow(player.position.y - worldY, 2)
      );
