...
import { AnimationScheduler } from '../../utils/animationOptimizations';
...
const PerformanceMonitorComponent: React.FC<{
  ...
  const animationScheduler = useRef(AnimationScheduler.getInstance());
  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      const animMetrics = animationScheduler.current.getPerformanceMetrics();
  ...
  const updatePerformanceGrade = useCallback((metrics: PerformanceMetrics) => {
    ...
    else if (metrics.memoryUsage > 40) score -= 5;
    
    // Render performance (20% weight)
    const slowRenderRate = (metrics.slowRenders / Math.max(metrics.totalRenders, 1)) * 100;
    if (slowRenderRate > 20) score -= 20;
    else if (slowRenderRate > 10) score -= 10;
    else if (slowRenderRate > 5) score -= 5;
    
    // Animation performance (10% weight)
    if (metrics.animationCount > 20) score -= 10;
    else if (metrics.animationCount > 10) score -= 5;
  ...
  const generateSuggestions = useCallback((metrics: PerformanceMetrics) => {
    ...
    if (metrics.fps < 30) {
      newSuggestions.push({
        ...
        description: `FPS is ${metrics.fps}, well below 60fps target. Consider enabling performance mode.`,
        action: () => {
          animationScheduler.current.forcePerformanceMode(true);
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Real-time performance metrics and optimization</p>
            </div>
          </div>
          
          <div className="flex items-center space-x-2">
            {/* Performance Grade */}
            <div className={`px-3 py-1 rounded-lg font-bold text-lg ${getGradeColor(performanceGrade)}`}>
              {performanceGrade}
            </div>
        ...
        <div className="p-4 space-y-6 max-h-[calc(100vh-120px)] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-800/50 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-white">{Math.round(metrics.memoryUsage)}%</div>
              ...
              <div className="mt-2 w-full bg-slate-700 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full transition-all duration-300 ${
                    ...
                    metrics.memoryUsage < 75 ? 'bg-yellow-400' : 'bg-red-400'
                  }`}
            ...
            <div className="bg-slate-800/50 rounded-lg p-4">
              ...
              <div className="text-xs text-slate-500">
                {metrics.slowRenders} slow ({Math.round((metrics.slowRenders / Math.max(metrics.totalRenders, 1)) * 100)}%)
              </div>
              <div className="mt-2 w-full bg-slate-700 rounded-full h-2">
                <div 
                  className="h-2 rounded-full bg-green-400 transition-all duration-300"
                  style={{ width: `${Math.max(100 - (metrics.slowRenders / Math.max(metrics.totalRenders, 1)) * 100, 0)}%` }}
                />
              </div>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4">
              ...
              <div className="mt-2 w-full bg-slate-700 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full transition-all duration-300 ${
                    metrics.cacheHitRate >= 95 ? 'bg-green-400' : 
                    metrics.cacheHitRate >= 85 ? 'bg-yellow-400' : 'bg-red-400'
                  }`}
          ...
          <div className="flex items-center justify-between p-4 bg-slate-800/30 rounded-lg">
            <div>
              ...
              <p className="text-sm text-slate-400">
                {isRecording ? 'Recording performance metrics...' : 'Start a 30-second performance recording'}
              </p>
            </div>
            <button
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        {suggestion.type === 'critical' && <AlertTriangle className="w-4 h-4 text-red-400" />}
                        {suggestion.type === 'warning' && <AlertTriangle className="w-4 h-4 text-yellow-400" />}
                        {suggestion.type === 'info' && <CheckCircle className="w-4 h-4 text-blue-400" />}
            ...
            <div className="bg-slate-800/50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`${animationScheduler.current.getPerformanceMetrics().isPerformanceMode ? 'text-yellow-400' : 'text-green-400'}`}>
                    {animationScheduler.current.getPerformanceMetrics().isPerformanceMode ? 'Enabled' : 'Disabled'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white">~{Math.round((metrics.bundleSize || 450) * 0.3)}KB</span>
                </div>
              </div>
            </div>
          </div>
