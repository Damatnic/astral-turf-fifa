            ...
            <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`}>
                ...
                <g className="text-gray-600">
                    ...
                    {axisTicks.xTicks.map(tick => (
                        <g key={`x-tick-${tick.value}`}>
                            <line x1={tick.x} y1={padding.top} x2={tick.x} y2={height - padding.bottom} stroke="currentColor" strokeWidth="0.5" strokeDasharray="2 2" />
                    ...
                    <text x={width / 2} y={height - 10} textAnchor="middle" fontSize="12" fill="var(--text-secondary)" className="font-semibold capitalize">{xAxisLabel.replace(/([A-Z])/g, ' $1')}</text>
                    <text transform={`rotate(-90)`} x={-(height / 2)} y={15} textAnchor="middle" fontSize="12" fill="var(--text-secondary)" className="font-semibold capitalize">{yAxisLabel.replace(/([A-Z])/g, ' $1')}</text>
                </g>
             ...
             {tooltip && (
                ...
                >
                    ...
                    <p>{xAxisLabel}: {tooltip.point.x}</p>
