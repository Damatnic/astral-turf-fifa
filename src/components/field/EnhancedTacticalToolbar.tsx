...
const ToolButton: React.FC<{
  ...
  shortcut?: string;
  tooltip?: string;
}> = React.memo(({ label, tool, onClick, isActive, disabled = false, children, shortcut, tooltip }) => {
...
const FormationSelector: React.FC = React.memo(() => {
  ...
  const handleFormationChange = useCallback((team: 'home' | 'away', formationId: string) => {
    dispatch({
...
const EnhancedTacticalToolbar: React.FC = () => {
  ...
  const [activeSection, setActiveSection] = useState<'tools' | 'animation' | 'view' | 'formation'>('tools');
        ...
        `}>
          ...
          <div className="p-3">
            ...
            `}>
              ...
              {/* Color picker */}
              {(!isMobile || activeSection === 'tools') && (
                <ColorPicker currentColor={drawingColor || '#ffffff'} onColorChange={setColor} />
              )}
              ...
              {/* Formation selector */}
              {(!isMobile || activeSection === 'formation') && (
                <FormationSelector />
              )}
            </div>
          </div>
