  ...
  return (
      ...
      >
        <div className={`relative ${isHighlightedByAI ? 'ai-highlight' : ''} rounded-full`}>
            ...
            >
                ...
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-8 h-2.5 bg-gray-900/50 rounded-full border border-gray-500/50 flex items-center p-px" title={`Stamina: ${player?.stamina ?? 0}%`}>
                 ...
                 <div title={`Morale: ${player?.morale ?? 'Unknown'}`} className="absolute -top-1 -left-1 w-4 h-4 p-0.5 bg-gray-800 rounded-full border-2 border-white flex items-center justify-center">
        ...
        <p className="mt-1.5 text-xs font-semibold text-white bg-black/40 px-1.5 py-0.5 rounded-md whitespace-nowrap">
            <span className="text-gray-400 mr-1">#{player?.jerseyNumber ?? '?'}</span>{player?.name ?? 'Unknown Player'}
        </p>
      </div>
  );
});
