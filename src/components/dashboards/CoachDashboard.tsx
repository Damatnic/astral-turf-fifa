...
const InfoCard: React.FC<{ title: string, icon: React.ReactNode, onClick: () => void, children: React.ReactNode }> = ({ title, icon, onClick, children }) => (
    <div onClick={onClick} className="bg-gray-700/50 p-4 rounded-lg cursor-pointer hover:bg-gray-700/80 transition-colors border border-transparent hover:border-teal-500/50">
                ...
                <div className="p-6 overflow-y-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <InfoCard title="Next Match" icon={<TrophyIcon className="w-5 h-5 text-yellow-400"/>} onClick={() => navigate('/league-table')}>
                                {nextFixture ? (
                                    <>
                                        <p>vs <span className="font-bold">{nextFixture.homeTeam === 'Astral FC' ? nextFixture.awayTeam : nextFixture.homeTeam}</span> ({nextFixture.homeTeam === 'Astral FC' ? 'H' : 'A'})</p>
                                        <p className="text-xs text-gray-400 mt-1">Ready your tactics for the upcoming game.</p>
                                    </>
                                ) : (
                        ...
                        <InfoCard title="Squad Status" icon={<UsersIcon className="w-5 h-5 text-gray-300"/>} onClick={() => navigate('/medical-center')}>
                             <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-lg font-bold">{players.filter(p => p.team === 'home').length}</p>
                         ...
                         <InfoCard title="Board Objective" icon={<ShieldCheck className="w-5 h-5 text-gray-300"/>} onClick={() => navigate('/board-objectives')}>
                             {criticalObjective ? (
                                <>
                                 ...
                                 <p className="text-gray-400">All critical objectives are currently met. Keep up the good work.</p>
                             )}
                        </InfoCard>
                    </div>
                         ...
                         <div className="bg-gray-700/50 p-4 rounded-lg">
                             ...
                             <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold text-white">${finances.home.transferBudget.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-400">Wage Budget (rem.)</span>
                                    <span className="font-semibold text-white">${(finances.home.wageBudget - players.filter(p=>p.team==='home').reduce((s,p)=>s+(p.contract.wage || 0),0)).toLocaleString()}</span>
                                </div>
                             </div>
                             <button onClick={() => navigate('/finances')} className="w-full mt-4 text-xs text-center py-1.5 bg-teal-600/50 hover:bg-teal-600 rounded-md">View Details</button>
                        </div>
                    </div>
                </div>
            </div>
