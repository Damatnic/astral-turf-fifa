...
const TrainingDay: React.FC<{ day: string; schedule: WeeklySchedule[keyof WeeklySchedule] }> = ({ day, schedule }) => (
            ...
            <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]">
                <div className="flex justify-between items-center p-4 border-b border-gray-700">
                     <div className="flex items-center">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-400">{role.name} | {player.age} years old</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-1 space-y-2">
                         <h3 className="font-bold text-lg text-gray-200 flex items-center mb-2"><StarIcon className="w-5 h-5 mr-2 text-yellow-400" /> My Attributes</h3>
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div>
                            <h3 className="font-bold text-lg text-gray-200 flex items-center mb-2"><DumbbellIcon className="w-5 h-5 mr-2" /> My Weekly Schedule</h3>
                         ...
                         <div>
                            <h3 className="font-bold text-lg text-gray-200 flex items-center mb-2"><HeartHandshakeIcon className="w-5 h-5 mr-2" /> My Contract</h3>
                            <div className="bg-gray-700/50 p-4 rounded-lg space-y-2">
                                {player.contract.clauses.map(clause => (
                                     <div key={clause.id} className="flex justify-between items-center text-sm">
                                         ...
                                         <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${clause.status === 'Met' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>{clause.status}</span>
                                     </div>
                                ))}
                            </div>
                        </div>
                        <div>
                            <h3 className="font-bold text-lg text-gray-200 flex items-center mb-2"><AwardIcon className="w-5 h-5 mr-2" /> My Skill Badges</h3>
                            <div className="bg-gray-700/50 p-4 rounded-lg">
                                {completedChallenges.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {completedChallenges.map(challenge => (
                                            <div key={challenge.id} className="flex items-center p-2 rounded-md" style={{backgroundColor: challenge.color + '20'}} title={challenge.description}>
                                    ...
                                    <p className="text-sm text-center text-gray-500">No badges earned yet. Keep training!</p>
                                )}
                            </div>
                        </div>
                    </div>
