...
import { UserXIcon as UserIcon, CalendarIcon as Calendar, MessageSquareIcon as MessageCircle, ChartLineIcon as TrendingUp, HeartIcon as Heart, MoneyIcon as DollarSign, FileTextIcon as FileText } from '../ui/icons';
...
const FamilyDashboard: React.FC = () => {
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Your family account is not yet linked to a player. Please contact the coach to set up the association.
          </p>
        </div>
      </div>
    );
  ...
  const upcomingEvents = [
    ...
    { type: 'Match', date: 'Saturday, 2:00 PM', location: 'Home Stadium', opponent: 'vs Quantum Rovers' },
  ...
  const familyNotifications = [
    { title: 'Monthly Report Available', message: `${mainPlayer.name}'s performance report for this month is ready for review.`, time: '2 hours ago', type: 'report' },
    { title: 'Training Schedule Updated', message: 'Next week\'s training schedule has been updated. Please check for any conflicts.', time: '1 day ago', type: 'schedule' },
    { title: 'Fee Payment Due', message: 'Monthly training fees are due by the end of the week.', time: '2 days ago', type: 'payment' },
  ];
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[var(--bg-secondary)] rounded-lg p-6 border border-[var(--border-color)]">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-semibold text-[var(--text-primary)]">{mainPlayer.name}'s Performance</h2>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center p-4 bg-[var(--bg-tertiary)] rounded-lg">
                  <div className="text-2xl font-bold text-purple-400">{mainPlayer.stats.matchesPlayed}</div>
            ...
            <div className="bg-[var(--bg-secondary)] rounded-lg p-6 border border-[var(--border-color)]">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-semibold text-[var(--text-primary)]">Development Progress</h2>
              </div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {Object.entries(mainPlayer.attributes)
                    ...
                    .slice(0, 3)
                    .map(([attr, value]) => (
                      <div key={attr} className="text-center p-3 bg-[var(--bg-tertiary)] rounded-lg">
                        <div className={`text-lg font-bold ${getAttributeColor(value)}`}>
                          {value}
                        </div>
                        <div className="text-xs capitalize text-[var(--text-secondary)]">{attr}</div>
                      </div>
                    ))}
                </div>
              </div>
            ...
            <div className="bg-[var(--bg-secondary)] rounded-lg p-6 border border-[var(--border-color)]">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-lg font-semibold text-[var(--text-primary)]">Family Notifications</h2>
              </div>
                  ...
                  <div key={index} className="p-3 bg-[var(--bg-tertiary)] rounded-lg">
                    <div className="flex items-start space-x-3">
                      <div className={`w-3 h-3 rounded-full mt-1.5 ${
                        notification.type === 'payment' ? 'bg-red-400' :
                        notification.type === 'report' ? 'bg-blue-400' :
                        'bg-yellow-400'
                      }`}></div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[var(--bg-secondary)] rounded-lg p-6 border border-[var(--border-color)]">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-lg font-semibold text-[var(--text-primary)]">Upcoming Schedule</h3>
              </div>
              <div className="space-y-3">
                {upcomingEvents.map((event, index) => (
                  <div key={index} className="p-3 bg-[var(--bg-tertiary)] rounded-lg">
                    <div className="text-sm font-medium text-[var(--text-primary)]">
                      {event.type}
                      {event.opponent && <span className="text-yellow-400 ml-1">{event.opponent}</span>}
                    </div>
                    <div className="text-xs text-[var(--text-secondary)] mt-1">
                      {event.date}
                    </div>
            ...
            <div className="bg-[var(--bg-secondary)] rounded-lg p-6 border border-[var(--border-color)]">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <div className="text-[var(--text-primary)] font-medium">Astral FC Training Center</div>
