...
const getPositionCompatibility = (role1: string, role2: string): number => {
  ...
  const compatible =
    compatibilityMap[role1]?.includes(role2) || compatibilityMap[role2]?.includes(role1);
...
const ChemistryVisualization: React.FC<ChemistryVisualizationProps> = ({
  ...
  const connections = useMemo(() => {
    ...
    const activePlayers = players.filter(p => p.position && p.position.x >= 0 && p.position.y >= 0);
