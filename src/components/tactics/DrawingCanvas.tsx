...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({
   ...
   */
  const screenToField = useCallback((clientX: number, clientY: number): { x: number; y: number } | null => {
   ...
   */
  const svgToField = useCallback((e: React.MouseEvent<SVGSVGElement>): { x: number; y: number } | null => {
    if (!svgRef.current) return screenToField(e.clientX, e.clientY);
  ...
  const generateSmoothPath = useCallback((points: { x: number; y: number; pressure?: number }[]) => {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    e.preventDefault();
  }, [disabled, drawingTool, drawingColor, svgToField, performanceMode, cursorControls, isMobile, isTablet]);

  // Find shape at specific point for selection
  const findShapeAtPoint = useCallback((point: { x: number; y: number }): DrawingShape | null => {
    ...
    for (const shape of drawings) {
      if (shape.tool === 'text' && shape.points.length > 0) {
        ...
        if (distance < 2) return shape; // 2% tolerance for text
      }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const timeDiff = now - (lastPoint.timestamp || currentShape.timestamp);
    const distance = Math.sqrt(
      Math.pow(point.x - lastPoint.x, 2) + Math.pow(point.y - lastPoint.y, 2)
    );

    let updatedPoints = [...currentShape.points];

    switch (currentShape.tool) {
      case 'pen':
        // For pen tool, add continuous points with smoothing
        if (timeDiff > 10 && distance > 0.5) { // Throttle and minimum distance
          updatedPoints.push({ ...point, pressure: 0.5, timestamp: now });
        }
        break;
      case 'arrow':
      case 'line':
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent<SVGSVGElement>) => {
    if (disabled || !gestureEnabled || isMultiTouchGesture) return;
  ...
  }, [disabled, gestureEnabled, isMultiTouchGesture, touchState, screenToField, getTouchPressure, handleMouseMove]);
   ...
   */
  const handleShapeClick = useCallback((shape: DrawingShape, e?: React.MouseEvent) => {
   ...
   */
  const renderShape = useCallback((shape: DrawingShape, isPreview = false) => {
    ...
    const shapeElement = (() => {
      ...
      case 'zone':
        ...
        case 'pen':
          ...
          const pathData = tool === 'pen' && points.length > 3 ?
      ...
      case 'text':
        ...
        return (
          <text
            ...
            opacity={opacity}
            className={`pointer-events-none ${drawingTool === 'select' ? 'cursor-pointer' : ''}`}
            onClick={() => handleShapeClick(shape)}
          >
            {text}
          </text>
  ...
  return (
    <>
      ...
      >
        ...
        <g className="drawings">
          {drawings.map(shape => renderShape(shape))}
        </g>

        {/* Render current drawing preview */}
        {currentShape && (
          <g className="current-drawing">
            {renderShape(currentShape, true)}
          </g>
        )}
      </svg>
