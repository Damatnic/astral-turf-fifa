...
type DrawingTool = 'select' | 'pen' | 'arrow' | 'rectangle' | 'circle' | 'triangle' | 'zone' | 'text' | 'eraser';
...
const TacticalDrawingTools: React.FC<TacticalDrawingToolsProps> = ({
  ...
  const addToHistory = useCallback((newShapes: DrawingShape[]) => {
    ...
    switch (shape.type) {
      ...
      case 'arrow':
        if (shape.points.length >= 2) {
          ...
          return (
            <g key={key} onClick={() => handleShapeClick(shape.id)} className={activeTool === 'eraser' ? 'cursor-pointer' : ''}>
              <line
                x1={startX}
                y1={startY}
                x2={endX}
