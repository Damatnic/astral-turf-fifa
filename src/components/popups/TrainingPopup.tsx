...
import type { TrainingDrill, TrainingDrillCategory, TrainingIntensity, DailySchedule, WeeklySchedule, Player } from '../../types';
    ...
    >
        <div className="flex justify-between items-start mb-2">
            ...
            <span className={`px-2 py-1 text-xs rounded-full ${
                drill.intensity === 'high' ? 'bg-red-600 text-white' :
                drill.intensity === 'medium' ? 'bg-yellow-600 text-white' :
        ...
        <div className="mt-2 flex justify-between text-xs opacity-75">
            ...
            <span>Morale: {drill.moraleEffect > 0 ? '+' : ''}{drill.moraleEffect}</span>
        </div>
    </div>
);
...
const TrainingPopup: React.FC = () => {
    ...
    const [selectedDrillType, setSelectedDrillType] = useState<'warmup' | 'main' | 'cooldown'>('warmup');
    const [selectedCategory, setSelectedCategory] = useState<TrainingDrillCategory>('attacking');
    const [weeklySchedule, setWeeklySchedule] = useState<WeeklySchedule>({
        monday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: false },
        tuesday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: false },
        wednesday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: false },
        thursday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: false },
        friday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: false },
        saturday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: true },
        sunday: { morning: { warmup: null, main: null, cooldown: null }, afternoon: { warmup: null, main: null, cooldown: null }, isRestDay: true },
    });
    ...
    const averageStamina = homeTeamPlayers.reduce((sum, p) => sum + p.stamina, 0) / homeTeamPlayers.length;
    const averageFatigue = homeTeamPlayers.reduce((sum, p) => sum + p.fatigue, 0) / homeTeamPlayers.length;

    return (
        <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm flex items-center justify-center p-4" onClick={handleClose}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-5xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]">
                {/* Header */}
                ...
                <div className="p-4 bg-gray-900/50 border-b border-gray-700">
                    <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div>
                            <div className="text-2xl font-bold text-orange-400">{Math.round(averageFatigue)}%</div>
                ...
                <div className="flex-shrink-0 px-4 border-b border-gray-700">
                    <div className="flex space-x-1">
                        <TabButton active={activeTab === 'schedule'} onClick={() => setActiveTab('schedule')}>Weekly Schedule</TabButton>
                        <TabButton active={activeTab === 'drills'} onClick={() => setActiveTab('drills')}>Drill Library</TabButton>
                        <TabButton active={activeTab === 'individual'} onClick={() => setActiveTab('individual')}>Individual Focus</TabButton>
                                ...
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                    {filteredDrills.map(drill => (
                                        <DrillCard
                                            ...
                                            isSelected={weeklySchedule[selectedDay][selectedSession][selectedDrillType] === drill.id}
                                        />
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {TRAINING_DRILLS.filter(drill => drill.category === selectedCategory).map(drill => (
                                        <DrillCard
                                            key={drill.id}
                                            drill={drill}
                                            onSelect={() => {}}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {['speed', 'passing', 'tackling', 'shooting', 'dribbling', 'positioning', 'stamina'].map(attribute => (
                                                <button
                                                    key={attribute}
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {[
                                        ...
                                        { name: 'Defensive Shape', description: 'Build solid defensive foundations', color: 'bg-blue-600' },
                                        { name: 'Physical Conditioning', description: 'Improve fitness and stamina levels', color: 'bg-orange-600' },
                                        { name: 'Technical Development', description: 'Enhance ball control and passing', color: 'bg-green-600' },
                                        { name: 'Tactical Discipline', description: 'Focus on formation and positioning', color: 'bg-purple-600' },
                                        { name: 'Pre-Season', description: 'Comprehensive preparation schedule', color: 'bg-teal-600' },
                                    ].map(template => (
                                        <div key={template.name} className="p-4 bg-gray-700/50 border border-gray-600 rounded-lg hover:bg-gray-700 transition-colors">
