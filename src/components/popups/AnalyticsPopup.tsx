    ...
    <div className="bg-gray-700/50 border border-gray-600 rounded-lg p-4">
        ...
        {change !== undefined && (
            <div className={`flex items-center text-sm ${change >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {change >= 0 ? <ArrowIcon className="w-4 h-4 mr-1 rotate-0" /> : <ArrowIcon className="w-4 h-4 mr-1 rotate-180" />}
        ...
        <tr className="border-b border-gray-700 hover:bg-gray-700/30">
            ...
            <td className="px-4 py-3 text-center">
                <span className={`px-2 py-1 text-xs rounded-full ${
                    player.form === 'Excellent' ? 'bg-green-600 text-white' :
                    player.form === 'Good' ? 'bg-blue-600 text-white' :
                    player.form === 'Average' ? 'bg-yellow-600 text-white' :
                    player.form === 'Poor' ? 'bg-orange-600 text-white' :
...
const AnalyticsPopup: React.FC = () => {
    ...
    const teamStats = {
        ...
        averageAge: Math.round(homeTeamPlayers.reduce((sum, p) => sum + p.age, 0) / homeTeamPlayers.length),
        averageStamina: Math.round(homeTeamPlayers.reduce((sum, p) => sum + p.stamina, 0) / homeTeamPlayers.length),
        playersInGoodForm: homeTeamPlayers.filter(p => p.form === 'Excellent' || p.form === 'Good').length,
    };
    ...
    const topAssists = homeTeamPlayers
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm flex items-center justify-center p-4" onClick={handleClose}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-7xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]">
                {/* Header */}
                ...
                <div className="flex-shrink-0 px-4 border-b border-gray-700">
                    <div className="flex space-x-1">
                        ...
                        <TabButton active={activeTab === 'players'} onClick={() => setActiveTab('players')}>Player Stats</TabButton>
                        <TabButton active={activeTab === 'tactical'} onClick={() => setActiveTab('tactical')}>Tactical Analysis</TabButton>
                    </div>
                </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <StatCard title="Team Fitness" value={`${teamStats.averageStamina}%`} change={-3} />
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {topScorers.map((player, index) => (
                                            <div key={player.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-md">
                                                <div className="flex items-center">
                                                ...
                                                <div className="text-right">
                                                    <div className="text-xl font-bold text-green-400">{player.stats.goals}</div>
                                    ...
                                    <div className="space-y-3">
                                        {topAssists.map((player, index) => (
                                            <div key={player.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-md">
                                                ...
                                                <div className="text-right">
                                                    <div className="text-xl font-bold text-blue-400">{player.stats.assists}</div>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="bg-gray-700/30 rounded-lg p-4">
                                    <LineChart data={performanceData.monthlyGoals.map((d, i) => ({ x: i + 1, y: d.goals }))} />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div className="bg-gray-700/30 rounded-lg p-4">
                                        <BarChart data={performanceData.monthlyGoals.map(d => ({ label: d.month, value: d.goals }))} />
                                    </div>
                                </div>
                                        ...
                                        <table className="w-full">
                                            <thead className="bg-gray-600">
                                                <tr>
                                                    ...
                                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Matches</th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Goals</th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Assists</th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Rating</th>
                                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">Form</th>
