...
import type { Formation, FormationSlot, PositionRole } from '../../types';
import { PLAYER_ROLES } from '../../constants';

const RoleToken: React.FC<{
  role: { id: string; name: string; abbreviation: string };
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (type === 'new_role') {
      ...
      if (emptySlotIndex !== -1) {
        ...
        const existingSlot = newSlots[emptySlotIndex];
        if (existingSlot) {
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]"
      >
        ...
        <div className="p-4 flex flex-grow min-h-0">
          ...
          >
            ...
            {slots
              .filter(s => s.defaultPosition.x >= 0)
              .map(slot => (
                <div
                  key={slot.id}
                  draggable
        ...
        <div className="p-4 border-t border-gray-700 flex items-center justify-between">
          ...
          <button
            onClick={handleSave}
            className="px-6 py-2 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-md flex items-center"
          >
