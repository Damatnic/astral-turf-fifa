    ...
    return (
        ...
        >
            <div className="flex items-center truncate">
                <div className="w-5 h-5 rounded-full flex items-center justify-center font-bold text-xs mr-2 flex-shrink-0" style={{ backgroundColor: player.teamColor, fontSize: '10px' }}>
                    {player.jerseyNumber}
                </div>
...
const SlotActionMenu: React.FC = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (isSlotActionMenuOpen && menuRef.current && !menuRef.current.contains(event.target as Node)) {
                 ...
                 <>
                    ...
                    <div className="space-y-1">
                        {trigger === 'drag' && targetPlayer && (
                            <>
                                <ActionButton onClick={(e) => { e.stopPropagation(); handleCompare(targetPlayer.id); }} title={`Compare ${sourcePlayer.name} vs ${targetPlayer.name}`}>
                                    <CompareIcon className="w-4 h-4 mr-2" />
                                    <span>Compare</span>
                                </ActionButton>
                                {canSwapOnDrag && (
                                    <ActionButton onClick={(e) => { e.stopPropagation(); handleDragSwap(); }} title={`Swap with ${targetPlayer.name}`}>
                                        <SwapIcon className="w-4 h-4 mr-2" />
                                        <span>Swap</span>
                                    </ActionButton>
                                )}
                                <ActionButton onClick={(e) => { e.stopPropagation(); handleAction('replace'); }} title={`Move ${targetPlayer.name} to bench`}>
                        ...
                        {trigger === 'click' && targetPlayer && (
                            <>
                                <ActionButton onClick={(e) => { e.stopPropagation(); setView('swap-list'); }} title="Swap this player">
                                    <SwapIcon className="w-4 h-4 mr-2" />
                                    <span>Swap with...</span>
                                </ActionButton>
                                <ActionButton onClick={(e) => { e.stopPropagation(); handleAction('bench'); }} title="Move to Bench">
                                    <TrashIcon className="w-4 h-4 mr-2" />
                                    <span>Move to Bench</span>
                                </ActionButton>
                                <ActionButton onClick={(e) => { e.stopPropagation(); handleOpenCompare(); }} title="Compare this player">
                                     <CompareIcon className="w-4 h-4 mr-2" />
                                     <span>Compare</span>
                                 </ActionButton>
                                 <ActionButton onClick={(e) => { e.stopPropagation(); handleAction('captain'); }} title={isCaptain ? "Remove Captain" : "Make Captain"}>
             ...
             {view === 'swap-list' && (
                <div className="flex flex-col flex-grow min-h-0">
                    <div className="flex items-center px-1 pb-2 mb-2 border-b border-gray-700 flex-shrink-0">
                        <button onClick={() => setView('main')} className="p-1 mr-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-full">
                    ...
                    <div className="space-y-3 overflow-y-auto pr-1 flex-grow">
                        {availablePlayers.onField.length > 0 && (
                            <div>
                                ...
                                <div className="space-y-1">
                                    {availablePlayers.onField.map(p => <PlayerSwapListItem key={p.id} player={p} onSwap={handleDirectSwap} onCompare={handleCompare} />)}
                                </div>
                            </div>
                        )}
                         {availablePlayers.benched.length > 0 && (
                            <div className={availablePlayers.onField.length > 0 ? "mt-3" : ""}>
                                ...
                                <div className="space-y-1">
                                    {availablePlayers.benched.map(p => <PlayerSwapListItem key={p.id} player={p} onSwap={handleDirectSwap} onCompare={handleCompare} />)}
                                </div>
                            </div>
                        )}
