                ...
                <div className="flex items-center">
                    ...
                    <div>
                        ...
                        <div className="flex items-center space-x-3 text-sm text-gray-400">
                            ...
                            <span>{roleInfo?.abbreviation || 'N/A'} - {roleInfo?.name || 'Unknown'}</span>
                        </div>
                    </div>
                </div>
...
const TransferMarketPopup: React.FC = () => {
    ...
    const mockTransferPlayers: TransferPlayer[] = [
        {
            ...
            instructions: {},
            team: 'home',
            attributes: { speed: 85, passing: 88, tackling: 70, shooting: 82, dribbling: 90, positioning: 86, stamina: 83 },
            availability: { status: 'Available' },
            morale: 'Good',
            form: 'Excellent',
            developmentLog: [],
            contract: { clauses: [] },
            stats: { goals: 15, assists: 22, matchesPlayed: 34, shotsOnTarget: 45, tacklesWon: 25, saves: 0, passesCompleted: 420, passesAttempted: 500, careerHistory: [] },
            loan: { isLoaned: false },
            traits: ['Ambitious'],
            conversationHistory: [],
            stamina: 100,
        ...
        },
        {
            ...
            instructions: {},
            team: 'home',
            attributes: { speed: 78, passing: 85, tackling: 92, shooting: 55, dribbling: 74, positioning: 89, stamina: 87 },
            availability: { status: 'Available' },
            morale: 'Excellent',
            form: 'Good',
            developmentLog: [],
            contract: { clauses: [] },
            stats: { goals: 3, assists: 8, matchesPlayed: 38, shotsOnTarget: 12, tacklesWon: 156, saves: 0, passesCompleted: 680, passesAttempted: 750, careerHistory: [] },
            loan: { isLoaned: false },
        ...
        },
        {
            ...
            instructions: {},
            team: 'home',
            attributes: { speed: 91, passing: 79, tackling: 65, shooting: 84, dribbling: 93, positioning: 82, stamina: 89 },
            availability: { status: 'Available' },
            morale: 'Good',
            form: 'Average',
            developmentLog: [],
            contract: { clauses: [] },
            stats: { goals: 18, assists: 14, matchesPlayed: 32, shotsOnTarget: 52, tacklesWon: 18, saves: 0, passesCompleted: 280, passesAttempted: 350, careerHistory: [] },
            loan: { isLoaned: false },
            traits: ['Ambitious'],
            conversationHistory: [],
            stamina: 100,
    ...
    const filteredPlayers = useMemo(() => {
        return mockTransferPlayers.filter(player => {
            ...
            const matchesPrice = player.askingPrice >= filters.price.min && player.askingPrice <= filters.price.max;
    ...
    const handleScout = (playerId: string) => {
        const player = mockTransferPlayers.find(p => p.id === playerId);
        if (player) {
            dispatch({ type: 'GET_PLAYER_SCOUT_REPORT_START', payload: { playerId } });
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm flex items-center justify-center p-4" onClick={handleClose}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-6xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]">
                {/* Header */}
                ...
                <div className="p-4 border-b border-gray-700">
                    <div className="flex items-center space-x-4 mb-4">
                        ...
                        <select
                            ...
                            onChange={e => setFilters(prev => ({ ...prev, position: e.target.value as PositionRole | 'All' }))}
                ...
                <div className="flex-shrink-0 px-4 border-b border-gray-700">
                    <div className="flex space-x-1">
                        <TabButton active={activeTab === 'browse'} onClick={() => setActiveTab('browse')}>Browse Players</TabButton>
                        <TabButton active={activeTab === 'negotiations'} onClick={() => setActiveTab('negotiations')}>Active Negotiations</TabButton>
                        <TabButton active={activeTab === 'shortlist'} onClick={() => setActiveTab('shortlist')}>Shortlist</TabButton>
                        <TabButton active={activeTab === 'history'} onClick={() => setActiveTab('history')}>Transfer History</TabButton>
                    </div>
                </div>
                        ...
                        <div>
                            {filteredPlayers.length > 0 ? (
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-400">Try adjusting your search filters to find more players.</p>
                                </div>
                            )}
                        </div>
                    )}
