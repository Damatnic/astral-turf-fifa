...
const PlayerList: React.FC<{ title: string; players: Player[]; captainId: string | null }> = ({
  title,
  players,
  captainId,
}) => (
  <div>
    <h4 className="text-sm font-bold uppercase tracking-wider text-gray-400 border-b border-gray-600 pb-1 mb-2">
      {title}
    </h4>
      ...
      {(players?.length ?? 0) > 0 ? (
        players.map(player => (
          <li key={player?.id} className="flex items-center">
            <span className="w-6 text-right mr-3 font-mono text-gray-400">
              {player?.jerseyNumber ?? '?'}
            </span>
            <span className="font-semibold">
              {player?.name ?? 'Unknown Player'} {player?.id === captainId && '(C)'}
            </span>
          </li>
        ))
      ) : (
...
const PrintableLineup = forwardRef<HTMLDivElement, PrintableLineupProps>(
  ({ players, formations, activeFormationIds, captainIds }, ref) => {
      ...
      return (
        <div ref={ref} className="w-[800px] bg-slate-800 text-white p-8 font-sans">
          <div className="text-center text-gray-400">
            <p>Formation data not available</p>
          </div>
        </div>
      );
    }
        ...
        <header className="flex items-center justify-between pb-4 border-b-2 border-slate-600">
          ...
          <div className="text-right">
            <p className="font-bold text-lg">
              {homeFormation?.name ?? 'Unknown'} vs {awayFormation?.name ?? 'Unknown'}
            </p>
