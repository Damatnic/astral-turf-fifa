...
export const TouchFirstTacticsBoard: React.FC<TouchFirstTacticsBoardProps> = ({
  ...
  const quickActionButtons = [
    {
      icon: <ZoomIn />,
      action: () => setViewport(prev => ({ ...prev, scale: Math.min(prev.scale * 1.2, 3) })),
    },
    {
      icon: <ZoomOut />,
      action: () => setViewport(prev => ({ ...prev, scale: Math.max(prev.scale * 0.8, 0.5) })),
    },
    {
      icon: <RotateCcw />,
      action: () => setViewport({ ...viewport, scale: 1, offsetX: 0, offsetY: 0 }),
    },
    {
      icon: <Maximize2 />,
      action: () => setMobileUI(prev => ({ ...prev, compactMode: !prev.compactMode })),
    },
  ];
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-green-900 to-blue-900 relative overflow-hidden">
      {/* Mobile Header */}
      <div
        className={`absolute top-0 left-0 right-0 z-50 transition-transform ${
            ...
            .map(player => (
              <div
                ...
                className={`absolute transform -translate-x-1/2 -translate-y-1/2 transition-all duration-200 ${
                  touchState.selectedPlayer === player.id ? 'scale-125 z-20' : 'z-10'
                }`}
                style={{
        ...
        <div className="flex items-center justify-between text-white text-xs">
          ...
          <div className="flex items-center gap-2">
            {touchState.selectedPlayer && (
              <Badge className="bg-yellow-500/20 text-yellow-400">Selected</Badge>
            )}
            {mobileUI.hapticEnabled && (
              <Badge className="bg-green-500/20 text-green-400">Haptic</Badge>
            )}
          </div>
        </div>
      </div>
