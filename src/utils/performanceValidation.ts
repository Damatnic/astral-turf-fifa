...
export class CoreWebVitalsMonitor {
  ...
  private recordMetric(
    ...
    window.dispatchEvent(new CustomEvent('catalyst:performance-metric', {
      detail: metric
    }));
  ...
  private generatePerformanceReport() {
    ...
    Object.entries(metrics).forEach(([key, metric]) => {
      if (metric) {
        ...
        console.log(`${emoji} ${key.toUpperCase()}: ${metric.value.toFixed(2)}ms (${metric.rating})`);
      }
    });
    ...
    // Emit report event
    window.dispatchEvent(new CustomEvent('catalyst:performance-report', {
...
export class RealUserMonitoring {
  ...
  trackEvent(name: string, data: any) {
    ...
    // Emit RUM event
    window.dispatchEvent(new CustomEvent('catalyst:rum-event', {
      detail: event
    }));
  }
...
export function initializePerformanceMonitoring() {
  ...
  window.addEventListener('catalyst:performance-metric', (event: CustomEvent) => {
