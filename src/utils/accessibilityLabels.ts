...
export const accessibilityLabels: AccessibilityLabels = {
  ...
  input: (fieldName: string, required = false, format?: string) => {
    ...
    if (format) label += `. Expected format: ${format}`;
    return label;
  },
  
  select: (fieldName: string, currentValue?: string) => {
    let label = `${fieldName} dropdown`;
    if (currentValue) label += `. Currently selected: ${currentValue}`;
    return label;
  },
  ...
  tacticalBoard: {
    field: 'Interactive soccer field for tactical planning. Use arrow keys to navigate, space to select, and enter to activate elements.',
    ...
    dragInstruction: 'To move this player: use arrow keys for fine positioning, or drag with mouse to new location',
...
export const keyboardInstructions = {
  ...
  tacticalBoard: {
    playerMovement: 'Arrow keys: Move player in small increments. Shift + Arrow: Move in larger steps',
    selection: 'Tab: Cycle through players. Space: Select/deselect player. Enter: Open player details',
...
export const screenReaderAnnouncements = {
  ...
  actions: {
    ...
    analysisComplete: (rating: number) => `AI analysis complete. Formation rating: ${rating} out of 100`
  },
...
export const accessibilityHelpers = {
  ...
  generateAriaLabel: (
    ...
    if (secondaryInfo) label += `. ${secondaryInfo}`;
    if (interactionHint) label += `. ${interactionHint}`;
    return label;
  },
   ...
   */
  tableAccessibility: (caption: string, headers: string[], rowCount: number): {
    caption: string;
    summary: string;
  } => ({
    caption,
