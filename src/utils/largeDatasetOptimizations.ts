...
export class SpatialIndex {
  ...
  query(x: number, y: number, radius: number): string[] {
    ...
    for (let cellX = Math.floor(minX / this.cellSize); cellX <= Math.floor(maxX / this.cellSize); cellX++) {
      for (let cellY = Math.floor(minY / this.cellSize); cellY <= Math.floor(maxY / this.cellSize); cellY++) {
...
export class WorkerPool {
  ...
  private taskQueue: Array<{ task: any; resolve: (result: any) => void; reject: (error: any) => void }> = [];
...
export class LargeDatasetManager<T extends { id: string }> {
  ...
  private checkIndexRebuild(): void {
    if (this.changeCount - this.lastIndexRebuild > LARGE_DATASET_PERFORMANCE.INDEX_REBUILD_THRESHOLD) {
      this.rebuildIndices();
    }
  }
...
export function useLargeDataset<T extends { id: string }>(
  ...
  if (!managerRef.current) {
    ...
    if (initialData.length > 0) {
      managerRef.current.batch(() => {
        initialData.forEach(item => managerRef.current!.set(item));
      });
    }
  }
  ...
  const enableSpatialIndex = useCallback((cellSize?: number, bounds?: { width: number; height: number }) => {
