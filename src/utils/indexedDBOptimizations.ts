...
class CompressionEngine {
  ...
  static async compress(data: any): Promise<{ compressed: Uint8Array; originalSize: number; compressedSize: number }> {
...
export class AdvancedIndexedDB {
  ...
  async set<K extends StoreNames>(
    ...
    if (this.compressionEnabled && dataSize > 1024) { // 1KB threshold
      try {
        ...
        if (compressionResult.compressedSize < compressionResult.originalSize * 0.8) {
          ...
          compressed = true;
          console.log(`[IndexedDB] Compressed ${store}:${key} from ${compressionResult.originalSize} to ${compressionResult.compressedSize} bytes`);
        }
      } catch (error) {
  ...
  async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
    ...
    console.log(`[IndexedDB] Cleanup completed: ${deletedCount} items deleted, ${freedBytes} bytes freed`);
  ...
  private async updateCacheMetadata(sizeChange: number): Promise<void> {
    ...
    if (this.currentCacheSize > this.cacheSizeLimit) {
      console.warn(`[IndexedDB] Cache size limit exceeded (${this.currentCacheSize} > ${this.cacheSizeLimit}), triggering cleanup`);
      this.cleanup();
    }
  }
