...
export type Action =
  ...
  | {
      type: 'ADD_DEVELOPMENT_LOG';
      payload: { playerId: string; entry: Omit<DevelopmentLogEntry, 'id'> };
    }
  | {
      type: 'SET_INDIVIDUAL_TRAINING_FOCUS';
      payload: { playerId: string; focus: IndividualTrainingFocus | null };
    }
  | { type: 'ADD_CONTRACT_CLAUSE'; payload: { playerId: string; clauseText: string } }
  | {
      type: 'UPDATE_CONTRACT_CLAUSE';
      payload: { playerId: string; clauseId: string; status: ContractClause['status'] };
    }
  ...
  | {
      ...
      payload: { playerId: string; entry: Omit<CommunicationLogEntry, 'id' | 'date'> };
    }
  | {
  ...
  | {
      ...
      payload: { team: Team; tactic: keyof TeamTactics; value: TeamTacticValue };
    }
  ...
  | {
      type: 'SET_SET_PIECE_TAKER';
      payload: { team: Team; type: SetPieceType; playerId: string | null };
    }
  ...
  | {
      type: 'SET_PLAYER_RUN';
      payload: { stepId: string; playerId: string; points: { x: number; y: number }[] };
    }
  ...
  | {
      ...
      payload: { filter: keyof UIState['transferMarketFilters']; value: unknown };
    }
  // AI Actions
  ...
  | {
      ...
      payload: { response: ChatMessage; playerIdsToHighlight: readonly string[] };
    }
  ...
  | {
      ...
      payload: { fanConfidenceEffect: number; teamMoraleEffect: number; narrativeId?: string };
    }
  ...
  | {
      ...
      payload: { inboxId: string; playerId: string; fee: number; wageContribution: number };
    }
  ...
  | {
      ...
      payload: { facility: keyof Omit<Stadium, 'capacity'>; team: Team };
    }
