import { FranchiseState, Action, NewsItem, InboxItem, TrainingPlanTemplate } from '../../types';
...
export const franchiseReducer = (draft: FranchiseState, action: Action): FranchiseState | void => {
  switch (action.type) {
    ...
    case 'ADVANCE_SEASON': {
        ...
        draft.gameWeek = 1;
        addInboxItem(draft, 'objective', `New Season: ${draft.season.year}`, 'A new season has begun. Check your board objectives and prepare your squad!');
        break;
    }
    case 'ADD_INBOX_ITEM':
        addInboxItem(draft, action.payload.type, action.payload.title, action.payload.content, action.payload.payload);
        break;
    ...
    case 'HIRE_STAFF': {
        ...
        if(draft.finances[action.payload.team].transferBudget >= cost) {
            ...
            addInboxItem(draft, 'finance', `New Staff Hired`, `${action.payload.staff.name} has been hired as the new ${action.payload.type}.`);
        } else {
             addInboxItem(draft, 'finance', `Hiring Failed`, `Insufficient funds to hire ${action.payload.staff.name}.`);
        }
        break;
    }
    ...
    case 'UPGRADE_STADIUM_FACILITY': {
        ...
        const upgradeCost = draft.stadium[team][facility] * (facility === 'trainingFacilitiesLevel' ? 5000000 : 3000000);
        if (draft.finances[team].transferBudget >= upgradeCost) {
            ...
            draft.stadium[team][facility]++;
             addInboxItem(draft, 'finance', `${facility} Upgraded`, `The ${facility} has been upgraded to level ${draft.stadium[team][facility]}.`);
        }
        break;
    }
    ...
    case 'SET_SPONSORSHIP_DEAL': {
        ...
        addInboxItem(draft, 'finance', `New Sponsorship Deal`, `A new deal has been signed with ${deal.name}.`);
        break;
    }
    ...
    case 'START_NEGOTIATION': {
        ...
        if(player) {
            draft.negotiationData = {
                playerId: player.id,
                conversation: [`Agent: "So, we're here to discuss ${player.name}'s future. What do you have in mind?"`],
    ...
    case 'DISSOLVE_MENTORING_GROUP': {
        ...
        draft.mentoringGroups[team] = draft.mentoringGroups[team].filter(g => g.mentorId !== mentorId);
        addInboxItem(draft, 'mentoring', 'Mentoring Group Dissolved', 'A mentoring group has been dissolved.');
        break;
    }
