...
const DugoutSection: React.FC<DugoutSectionProps> = ({
  title,
  players,
  team,
  selectedPlayerId,
  ...
  return (
    ...
    >
      <h3
        className={`text-xs font-bold uppercase text-center mb-2 ${team === 'home' ? 'text-blue-400' : 'text-red-400'}`}
      >
        {title}
      </h3>
...
const Dugout: React.FC = () => {
  ...
  const { activePlaybookItemId, activeStepIndex, selectedPlayerId, highlightedByAIPlayerIds } =
    uiState;
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>, team: 'home' | 'away') => {
    ...
    if (!playersOnFieldIds.has(playerId)) {
      ...
      // Player is coming from the field
