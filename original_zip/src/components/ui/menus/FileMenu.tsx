...
export const FileMenu: React.FC = () => {
  ...
  const VALID_TRAITS: PlayerTrait[] = [
    'Leader',
    'Ambitious',
    'Loyal',
    'Injury Prone',
  ...
  const handleNewProject = () => {
    if (
      confirm('Are you sure you want to start a new project? Any unsaved changes will be lost.')
    ) {
  ...
  const handleSaveProject = async () => {
    ...
    if (uiState.activeSaveSlotId) {
      ...
      uiDispatch({
        ...
        payload: { message: `Project "${slotName}" saved.`, type: 'success' },
      });
    } else {
      uiDispatch({ type: 'OPEN_MODAL', payload: 'loadProject' });
      uiDispatch({
        type: 'ADD_NOTIFICATION',
        payload: { message: 'Please select or create a save slot.', type: 'info' },
      });
    }
  };
        ...
        Papa.parse<any>(fileContents, {
          ...
          complete: results => {
            try {
              const newPlayers: Player[] = results.data.map((row, index) => {
                ...
                const availability: PlayerAvailability = {
                  status: ([
                    ...
                    'Suspended',
                    'International Duty',
                  ].includes(row.availabilityStatus)
                    ? row.availabilityStatus
                    : 'Available') as PlayerAvailabilityStatus,
                ...
                const traits = (row.traits?.split(',') || [])
                  .map((t: string) => t.trim())
                  .filter((t: string): t is PlayerTrait => VALID_TRAITS.includes(t as PlayerTrait));

                const potentialParts = (row.potential || '70-80')
                  .split('-')
                  .map((s: string) => parseInt(s.trim(), 10));
                const potential: [number, number] =
                  potentialParts.length === 2 &&
                  !isNaN(potentialParts[0]) &&
                  !isNaN(potentialParts[1])
                ...
                return {
                  ...
                  teamColor:
                    team === 'away'
                      ? uiState.teamKits.away.primaryColor
                      : uiState.teamKits.home.primaryColor,
                  attributes,
                  position: { x: -100, y: -100 },
                  availability,
                  morale: (['Excellent', 'Good', 'Okay', 'Poor', 'Very Poor'].includes(row.morale)
                    ? row.morale
                    : 'Okay') as PlayerMorale,
                  form: (['Excellent', 'Good', 'Average', 'Poor', 'Very Poor'].includes(row.form)
                    ? row.form
                    : 'Average') as PlayerForm,
                  stamina: 100,
              ...
              tacticsDispatch({
                type: 'UPDATE_PLAYERS',
                payload: [...tacticsState.players, ...newPlayers],
              });
              uiDispatch({
                type: 'ADD_NOTIFICATION',
                payload: {
                  message: `${newPlayers.length} players imported successfully.`,
                  type: 'success',
                },
              });
            } catch (parseErr) {
  ...
  const handleExportRoster = async () => {
    ...
    try {
      ...
      if (filePath) {
        ...
        uiDispatch({
          type: 'ADD_NOTIFICATION',
          payload: { message: 'Roster exported successfully.', type: 'success' },
        });
      }
    } catch (err) {
      uiDispatch({
  ...
  const handleExportPlaybook = async () => {
    ...
    try {
      ...
      if (filePath) {
        ...
        uiDispatch({
          type: 'ADD_NOTIFICATION',
          payload: { message: 'Playbook exported successfully.', type: 'success' },
        });
      }
    } catch (err) {
  ...
  const handleImportPlaybook = async () => {
    ...
    try {
      ...
      if (typeof selected === 'string') {
        ...
        uiDispatch({
          type: 'ADD_NOTIFICATION',
          payload: { message: 'Playbook loaded successfully.', type: 'success' },
        });
      }
    } catch (err) {
