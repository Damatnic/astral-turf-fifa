...
const TabButton: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode }> = ({
  ...
  <button
    onClick={onClick}
    className={`px-3 py-1.5 text-xs font-semibold rounded-t-md transition-colors ${active ? 'bg-gray-700 text-teal-300' : 'bg-gray-800 text-gray-400 hover:bg-gray-700/50'}`}
  >
    {children}
  </button>
);

const PlayerEditPopup: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (editingPlayerId) {
      ...
      uiDispatch({
        ...
        payload: { message: `${playerData.name} updated successfully.`, type: 'success' },
      });
    } else {
      tacticsDispatch({ type: 'ADD_PLAYER', payload: playerData });
      uiDispatch({
        type: 'ADD_NOTIFICATION',
        payload: { message: `${playerData.name} created successfully.`, type: 'success' },
      });
    }
    handleClose();
  };
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-3xl border border-gray-700/50 flex flex-col animate-fade-in-scale max-h-[90vh]"
      >
        <div className="flex justify-between items-center p-4 border-b border-gray-700">
          <h2 className="text-xl font-bold text-teal-400">
            {editingPlayerId ? `Edit ${playerData.name}` : 'Create New Player'}
          </h2>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto">
          {activeTab === 'details' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <label className="text-gray-400">Jersey #</label>
                <input
                  type="number"
                  name="jerseyNumber"
              ...
              <div>
                <label className="text-gray-400">Nationality (ISO 3166-1 alpha-2)</label>
                <input
                  type="text"
                  name="nationality"
              ...
              <div>
                ...
                <input
                  type="color"
                  name="teamColor"
                  value={playerData.teamColor}
                  onChange={handleChange}
                  className="w-full h-10 p-1 bg-gray-700 rounded mt-1"
                />
              </div>
            </div>
          )}
              ...
              {Object.entries(playerData.attributes).map(([key, value]) => (
                <div key={key}>
                  ...
                  <input
                    ...
                    value={value}
                    onChange={e =>
                      handleAttributeChange(key as keyof PlayerAttributes, parseInt(e.target.value))
                    }
                    className="w-full"
                  />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.values(DETAILED_PLAYER_INSTRUCTIONS).map(instr => (
                <div key={instr.name}>
                  ...
                  <select
                    value={
                      playerData.instructions[instr.name.toLowerCase().replace(/ /g, '_')] ||
                      'default'
                    }
                    onChange={e =>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                {skillChallenges.map(challenge => (
                  <label
                    key={challenge.id}
                    className="flex items-center p-2 bg-gray-700/50 rounded-md cursor-pointer"
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <div
                      className="w-6 h-6 rounded-md ml-3 mr-2 flex items-center justify-center text-sm"
