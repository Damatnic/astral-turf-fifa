...
const TacticsBoardPage: React.FC = () => {
  ...
  return (
    <div className="flex h-screen w-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div
        className={`flex-shrink-0 transition-all duration-300 ease-in-out backdrop-blur-sm ${isPresentationMode ? 'w-0 overflow-hidden' : 'w-80'}`}
      >
      ...
      <main className="flex-grow flex flex-col min-h-0 relative">
        ...
        <div
          className={`flex-grow flex items-center justify-center relative z-10 ${isPresentationMode ? 'p-0' : 'p-4'}`}
        >
