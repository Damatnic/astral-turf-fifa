...
import { getAIPlayerComparison } from '../../services/aiService';
...
const PlayerComparePopup: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    if (!player1 || !player2 || !activeFormation) {
      return;
    }
    dispatch({ type: 'GENERATE_AI_COMPARISON_START' });
    try {
      const result = await getAIPlayerComparison(
        player1,
        player2,
        activeFormation,
        ...
        <div className="p-6 flex-grow overflow-y-auto max-h-[80vh]">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <div className="p-2 bg-gray-700 rounded-md font-bold text-white truncate">
                {player1.name}
              </div>
            </div>
            <div>
