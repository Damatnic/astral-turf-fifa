...
import { getAgentNegotiationResponse } from '../../services/aiService';
...
const ContractNegotiationPopup: React.FC = () => {
  ...
  if (!negotiationData || !player) {
    return null;
  }

  const handleSendOffer = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!offer.trim() || isLoadingNegotiation) {
      return;
    }
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-lg border border-gray-700/50 flex flex-col animate-fade-in-scale h-[70vh]"
      >
        <div className="flex justify-between items-center p-4 border-b border-gray-700">
          <h2 className="text-xl font-bold text-teal-400 flex items-center">
            <HandshakeIcon className="w-5 h-5 mr-2" />
            Negotiating with {player.name}'s Agent
          </h2>
          <button
            onClick={() => franchiseDispatch({ type: 'END_NEGOTIATION' })}
        ...
        <div className="flex-grow p-4 overflow-y-auto space-y-4">
          {negotiationData.conversation.map((msg, i) => (
            <div
              key={i}
              className={`flex items-start gap-3 ${msg.startsWith('You:') ? 'justify-end' : ''}`}
            >
              <p
                className={`max-w-xs px-4 py-2 rounded-2xl text-sm ${msg.startsWith('You:') ? 'bg-blue-600 text-white' : 'bg-slate-700'}`}
              >
                {msg}
              </p>
            </div>
        ...
        <div className="p-3 border-t border-slate-700">
          <form onSubmit={handleSendOffer} className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={!offer.trim() || isLoadingNegotiation}
              className="px-4 py-2 bg-teal-600 hover:bg-teal-500 disabled:bg-slate-600 text-white font-bold rounded-lg"
            >
              Send
            </button>
          </form>
