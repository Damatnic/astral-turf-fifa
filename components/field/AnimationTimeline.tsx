...
import { PlaybookEvent } from '../../types';

const StepEventEditor: React.FC<{ stepIndex: number, event: PlaybookEvent | undefined, onClose: () => void }> = ({ stepIndex, event, onClose }) => {
    ...
    const handleSave = () => {
        if (description) {
            dispatch({ type: 'SET_PLAYBOOK_EVENT', payload: { stepIndex, event: { type, description } } });
        }
        onClose();
    };
    ...
    return (
        <div className="absolute bottom-full mb-2 w-64 bg-slate-900 border border-slate-700 p-3 rounded-lg shadow-xl z-30">
            ...
            <div className="space-y-2">
                <select value={type} onChange={(e) => setType(e.target.value as any)} className="w-full p-1.5 text-sm bg-gray-700 border border-gray-600 rounded-md">
                    <option>Goal</option>
            ...
            <div className="flex justify-between mt-3">
                <button onClick={handleRemove} className="text-xs px-2 py-1 rounded-md text-red-400 hover:bg-red-500/20">Remove</button>
                <div className="space-x-2">
                    <button onClick={onClose} className="text-xs px-2 py-1 rounded-md hover:bg-gray-600">Cancel</button>
...
const StepButton: React.FC<{ index: number }> = ({ index }) => {
    ...
    return (
        <div className="relative">
            ...
            {isEditingEvent && <StepEventEditor stepIndex={index} event={event} onClose={() => setIsEditingEvent(false)} />}
        </div>
    );
};
...
const AnimationTimeline: React.FC = () => {
    ...
    return (
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-slate-800/70 backdrop-blur-sm border border-slate-600/50 rounded-lg shadow-2xl flex items-center p-2 space-x-2 z-20 animate-fade-in-scale" style={{animationDuration: '0.3s'}}>
