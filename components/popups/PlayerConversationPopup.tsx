...
import { getAIPlayerConversationResponse } from '../../services/aiService';
...
const PlayerConversationPopup: React.FC = () => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        dispatch({ type: 'SEND_PLAYER_MESSAGE_START', payload: { playerId: player.id, message: userMessage } });
        setMessage('');
                ...
                <div className="flex-grow p-4 overflow-y-auto space-y-4">
                    {player.conversationHistory?.map((msg) => (
                        <div key={msg.id} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                            {msg.sender === 'ai' && (
                                <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{backgroundColor: player.teamColor}}>
                                    <span className="font-bold">{player.jerseyNumber}</span>
                                </div>
                            )}
                            <div className={`max-w-xs md:max-w-sm px-4 py-2 rounded-2xl ${msg.sender === 'user' ? 'bg-blue-600 text-white rounded-br-lg' : 'bg-slate-700 text-slate-200 rounded-bl-lg'}`}>
