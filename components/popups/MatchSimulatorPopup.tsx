...
import { MatchEvent, MatchCommentary, MatchResult } from '../../types';
...
const MatchSimulatorPopup: React.FC = () => {
    ...
    const { players, formations, activeFormationIds, teamTactics, tacticalFamiliarity, chemistry } = tacticsState;
    ...
    const homeScore = simulationTimeline.filter(e => 'type' in e && e.type === 'Goal' && e.team === 'home').length;
    const awayScore = simulationTimeline.filter(e => 'type' in e && e.type === 'Goal' && e.team === 'away').length;
    ...
    const renderEvent = (event: MatchEvent | MatchCommentary) => {
        if ('type' in event) { // MatchEvent
            const Icon = event.type === 'Goal' ? GoalIcon : event.type.includes('Card') ? CardIcon : MedicalCrossIcon;
            const color = event.team === 'home' ? 'text-blue-400' : 'text-red-400';
            return <p><Icon className={`inline w-4 h-4 mr-2 ${event.type === 'Goal' ? 'text-yellow-400' : ''}`} /> <b className={color}>[{event.team.toUpperCase()}]</b> {event.playerName} {event.description}</p>;
        }
