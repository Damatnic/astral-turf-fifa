...
import { getAISubstitutionSuggestion } from '../../services/aiService';
...
const AISubstitutionSuggestionPopup: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 backdrop-blur-sm flex items-center justify-center p-4" onClick={handleClose}>
            <div onClick={e => e.stopPropagation()} className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-md border border-gray-700/50 flex flex-col animate-fade-in-scale">
                <div className="flex justify-between items-center p-4 border-b border-gray-700">
                    <h2 className="text-xl font-bold text-teal-400 flex items-center"><BrainCircuitIcon className="w-5 h-5 mr-2" />AI Substitution Suggestion</h2>
                    <button onClick={handleClose} className="p-1 rounded-full text-gray-400 hover:bg-gray-700"><CloseIcon className="w-5 h-5" /></button>
                </div>
                <div className="p-6">
                    {isLoadingAISubSuggestion && <div className="flex justify-center py-8"><LoadingSpinner /></div>}
                    {aiSubSuggestionData && playerIn && playerOut && (
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-end space-x-2 pt-2">
                                <button onClick={handleClose} className="px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded text-sm">Decline</button>
                                <button onClick={handleMakeSubstitution} className="px-4 py-2 bg-teal-600 hover:bg-teal-500 rounded text-sm font-semibold">Accept</button>
                            </div>
                        </div>
                    )}
                </div>
