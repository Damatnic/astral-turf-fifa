...
import { getTeamTalkOptions } from '../../services/aiService';
import { TeamTalkOption } from '../../types';
...
const TeamTalkPopup: React.FC = () => {
    ...
    const awayScore = uiState.simulationTimeline.filter(e => 'type' in e && e.type === 'Goal' && e.team === 'away').length;
    ...
    useEffect(() => {
        if (!teamTalkData && !isLoadingTeamTalk) {
            ...
            getTeamTalkOptions(teamPlayers, opponentName, isHalftime, currentScore, settings.aiPersonality)
                .then(data => dispatch({ type: 'GET_TEAM_TALK_OPTIONS_SUCCESS', payload: data }))
                .catch(err => {
                    console.error(err);
                    dispatch({ type: 'GET_TEAM_TALK_OPTIONS_FAILURE' });
                    dispatch({ type: 'ADD_NOTIFICATION', payload: { message: `AI Error: ${err.message}`, type: 'error' } });
                });
        }
                ...
                <div className="p-6 text-gray-300 min-h-[200px]">
                    ...
                    {teamTalkData && (
                        <div className="space-y-3">
                            <p className="text-sm text-gray-400 mb-4 text-center">Your assistant suggests the following approaches:</p>
