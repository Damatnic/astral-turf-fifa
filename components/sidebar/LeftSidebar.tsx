...
import { AddPlayerIcon, FormationIcon, GameplanIcon, HealthIcon, PencilIcon, ShirtIcon, TrashIcon, CaptainIcon, SparklesIcon, LoadingSpinner, SearchIcon, CopyIcon, UserXIcon, SlidersHorizontalIcon, ChartLineIcon, CrosshairIcon, LibraryIcon, UsersIcon } from '../ui/icons';
import { Player, Team, PlayCategory, PLAY_CATEGORIES, PlaybookItem, PositionRole, PlayerAvailabilityStatus } from '../../types';
import { getAIFormationSuggestion } from '../../services/aiService';
...
const getHealthColor = (status: PlayerAvailabilityStatus) => {
    switch (status) {
        ...
        case 'Major Injury': return 'bg-red-500';
...
const PlayerListItem: React.FC<PlayerListItemProps> = React.memo(({ player, isSelected, isCaptain, onSelect, onEdit, onDoubleClick, onDragStart, onAssignTeam }) => {
    ...
    return (
        ...
        >
            {isCaptain && (
                <div className="absolute -left-1.5 top-1/2 -translate-y-1/2 text-yellow-400" title="Captain">
                    <CaptainIcon className="w-4 h-4" />
                </div>
            )}
             <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm mr-3 border-2 border-black/20" style={{ backgroundColor: player.teamColor }}>
                {player.jerseyNumber}
            </div>
            <div className="flex-grow">
                <div className="flex items-center">
                    ...
                    {player.individualTrainingFocus && (
                        <div className="ml-1.5 text-teal-400" title={`Focus: ${player.individualTrainingFocus.attribute}`}>
                ...
                <div className="flex items-center mt-1">
                    ...
                    <div className={`w-2 h-2 rounded-full ${getHealthColor(player.availability.status)} mr-1`}></div>
                    <p className="text-xs text-gray-400 capitalize">{player.availability.status}</p>
                </div>
            </div>
            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                 <button onClick={(e) => handleTeamClick(e, 'home')} className={`w-5 h-5 text-xs font-bold rounded-sm flex items-center justify-center ${player.team === 'home' ? 'bg-blue-600 text-white ring-1 ring-white' : 'bg-gray-600 text-gray-300 hover:bg-blue-600'}`}>H</button>
                 <button onClick={(e) => handleTeamClick(e, 'away')} className={`w-5 h-5 text-xs font-bold rounded-sm flex items-center justify-center ${player.team === 'away' ? 'bg-red-600 text-white ring-1 ring-white' : 'bg-gray-600 text-gray-300 hover:bg-red-600'}`}>A</button>
                <div
                    onClick={onEdit}
...
const CreatePlaybookItemForm: React.FC<{ onSave: (name: string, category: PlayCategory) => void, onCancel: () => void }> = ({ onSave, onCancel }) => {
    ...
    return (
        <div className="p-2 bg-gray-900/50 rounded-md mb-2">
            ...
            <div className="flex justify-end mt-2 space-x-2">
                <button onClick={onCancel} className="text-xs px-2 py-1 rounded-md hover:bg-gray-600">Cancel</button>
                <button onClick={handleSave} className="text-xs px-2 py-1 rounded-md bg-teal-600 hover:bg-teal-500 font-semibold">Save</button>
            </div>
        </div>
    );
};
...
export const LeftSidebar: React.FC = () => {
    ...
    const sortedPlayers = useMemo(() => {
        ...
        return [...filtered].sort((a, b) => {
            const overallA = (Object.values(a.attributes) as number[]).reduce((s,v)=>s+v,0) / Object.keys(a.attributes).length;
            const overallB = (Object.values(b.attributes) as number[]).reduce((s,v)=>s+v,0) / Object.keys(b.attributes).length;
            switch(sortCriteria) {
                case 'jerseyNumber': return a.jerseyNumber - b.jerseyNumber;
                case 'overall': return overallB - overallA;
                case 'name':
    ...
    const playersOnFieldIds = useMemo(() => {
        if (activePlaybookItemId && activeStepIndex !== null) {
            ...
            if (step) {
                return new Set(Object.keys(step.playerPositions).filter(pId => step.playerPositions[pId].x > -1));
            }
        }
    ...
    }, [formations, activeFormationIds, activeTeamContext, activePlaybookItemId, activeStepIndex, playbook]);

    const onFieldPlayers = useMemo(() => sortedPlayers.filter(p => playersOnFieldIds.has(p.id)), [sortedPlayers, playersOnFieldIds]);
    const benchedPlayers = useMemo(() => sortedPlayers.filter(p => !playersOnFieldIds.has(p.id)), [sortedPlayers, playersOnFieldIds]);


    const handleFormationChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        dispatch({ type: 'SET_ACTIVE_FORMATION', payload: { formationId: e.target.value, team: activeTeam } });
    };
    ...
    const playbookByCategory = useMemo(() => {
        return Object.values(playbook).reduce((acc, item) => {
    ...
    return (
        <aside className="w-80 bg-[var(--bg-secondary)] flex flex-col p-4 shadow-lg overflow-y-auto h-full">
            {/* Formation Selector */}
            <div className="mb-4">
                <label htmlFor="formation-select" className="flex items-center text-sm font-medium text-gray-300 mb-2">
                    <FormationIcon className="w-4 h-4 mr-2" />
                    Formation ({activeTeamContext === 'away' ? 'Away' : 'Home'})
                </label>
                <div className="flex items-center space-x-2">
                    <select
                        ...
                        className="w-full p-2 bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:bg-gray-800 disabled:cursor-not-allowed"
                    >
                     ...
                     <button
                        ...
                        disabled={!!activePlaybookItemId}
                        className="p-2 bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-md hover:bg-red-600/50 disabled:bg-gray-800 disabled:cursor-not-allowed"
                     >
                 ...
                 <div className="mt-2">
                    <label className="flex items-center text-xs font-medium text-gray-400 mb-1">
                        <ChartLineIcon className="w-3 h-3 mr-1.5" />
                        Tactical Familiarity
                    </label>
                    <div className="w-full bg-gray-700 rounded-full h-2.5" title={`${familiarity}%`}>
                        <div className="bg-teal-500 h-2.5 rounded-full transition-all duration-500" style={{ width: `${familiarity}%` }}></div>
                    </div>
                </div>
            </div>
            ...
            <button
                ...
                className="w-full mb-4 bg-teal-600 hover:bg-teal-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-md transition-colors flex items-center justify-center"
            >
                {isSuggestingFormation ? <LoadingSpinner /> : (
                    <>
            ...
            <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                    <h3 className="flex items-center text-sm font-medium text-gray-300">
                        <GameplanIcon className="w-4 h-4 mr-2" />
                        Playbook
                    </h3>
                     <div className="flex items-center space-x-2">
                        <button onClick={() => uiDispatch({ type: 'OPEN_MODAL', payload: 'playbookLibrary' })} className="text-xs text-teal-400 hover:text-teal-300 font-semibold flex items-center"><LibraryIcon className="w-3 h-3 mr-1"/>Library</button>
                         {!isCreating && (
                            <button onClick={() => setIsCreating(true)} className="text-xs text-teal-400 hover:text-teal-300 font-semibold">+ Create</button>
                        )}
                     </div>
                </div>
                ...
                <div className="space-y-2">
                    {(Object.keys(playbookByCategory) as PlayCategory[]).sort().map(category => (
                         <div key={category}>
                            ...
                            >
                                 <span>{category} ({playbookByCategory[category].length})</span>
                                 <svg className={`w-4 h-4 transition-transform ${playbookCategories[category] ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                             </button>
            ...
            <div className="flex-grow flex flex-col min-h-0">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={handleAddNewPlayer} className="text-xs text-teal-400 hover:text-teal-300 font-semibold">
                        + Add Player
                    </button>
                </div>
                ...
                <div className="flex items-center space-x-2 mb-2">
                    <div className="relative flex-grow">
                        <input 
                            ...
                            onChange={(e) => uiDispatch({ type: 'SET_ROSTER_SEARCH_QUERY', payload: e.target.value })}
                            className="w-full p-2 pl-8 text-sm bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-md"
                        />
                    ...
                    <div className="relative">
                         <select
                             ...
                             className="appearance-none w-full p-2 text-sm bg-[var(--bg-tertiary)] border border-[var(--border-primary)] rounded-md"
                         >
                            <option value="name">Name</option>
                            <option value="jerseyNumber">Number</option>
                ...
                <div className="flex-grow overflow-y-auto space-y-3 pr-1">
                    <div>
                     ...
                     <div>
                        <h4 className="text-xs font-bold uppercase text-gray-500 px-2 mb-1">Bench ({benchedPlayers.length})</h4>
                        <ul className="space-y-1">
