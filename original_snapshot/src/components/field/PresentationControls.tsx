...
const EventDisplay: React.FC<{ event: PlaybookEvent }> = ({ event }) => {
  ...
  return (
    <div className="flex items-center text-sm">
      ...
      <span className="text-gray-300 ml-1.5 truncate" title={event?.description ?? ''}>
  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-slate-800/90 backdrop-blur-md border border-slate-600/50 rounded-xl shadow-2xl flex items-center p-3 space-x-4 z-30 animate-fade-in-scale">
      <button
        ...
        title="Previous Step"
        className="p-2 rounded-md text-gray-300 hover:bg-gray-700 disabled:text-gray-600 disabled:cursor-not-allowed"
      >
      ...
      <button
        ...
        className="p-3 rounded-full text-white bg-teal-600 hover:bg-teal-500 disabled:bg-gray-700 disabled:cursor-not-allowed transition-colors"
      >
      ...
      <button
        ...
        disabled={isAnimating || activeStepIndex === totalSteps - 1}
        title="Next Step"
        className="p-2 rounded-md text-gray-300 hover:bg-gray-700 disabled:text-gray-600 disabled:cursor-not-allowed"
      >
      ...
      <div className="flex flex-col items-center justify-center w-48">
        ...
        <div className="h-5 mt-1">{event && <EventDisplay event={event} />}</div>
      </div>
      ...
      <button
        ...
        className="p-2 rounded-md text-teal-400 hover:bg-gray-700"
      >
