...
async function executeCommand(command, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        if (!silent) log.success(`Command completed in ${duration}ms`);
        resolve({ code, output, duration });
      } else {
        if (!silent) log.error(`Command failed with code ${code}`);
    ...
    child.on('error', (error) => {
      clearTimeout(timeoutId);
      if (!silent) log.error(`Command error: ${error.message}`);
      reject(error);
    });
  });
}
...
function analyzeCoverage(coverageData) {
  ...
  let tacticalStats = { statements: 0, branches: 0, functions: 0, lines: 0 };
  let tacticalCount = 0;
...
function generateCoverageReport(analysis) {
  ...
  if (analysis.gaps.length > 0) {
    ...
    analysis.gaps.forEach(gap => {
      ...
      log.warning(`${fileDisplay} - ${gap.metric}: ${gap.actual.toFixed(2)}% (need ${gap.expected}%, gap: ${gap.gap.toFixed(2)}%)`);
    });
  } else {
...
function generateExecutionSummary(results, coveragePassed) {
  ...
  if (overallPassed) {
    ...
    if (!allCriticalPassed) log.error('Critical test phases failed');
    if (!coveragePassed) log.error('Coverage thresholds not met');
  }
  console.log('='.repeat(80));
