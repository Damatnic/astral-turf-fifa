  ...
  async testHealthEndpoint() {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        this.logTest('Health endpoint responds', false, `Status: ${response.statusCode}`);
      }
    } catch (error) {
    ...
    for (const endpoint of apiEndpoints) {
      try {
        ...
        if (response.statusCode === 200 || response.statusCode === 401) {
          ...
          this.logTest(`${endpoint.name} responds`, true, `Status: ${response.statusCode}`);
        } else {
          this.logTest(`${endpoint.name} responds`, false, `Status: ${response.statusCode}`);
        }
      } catch (error) {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        if (headers['content-encoding']) {
          this.logTest('Content compression enabled', true, headers['content-encoding']);
        } else {
          this.logTest('Content compression enabled', false, 'No content-encoding header');
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.statusCode === 200 && response.data?.checks?.database) {
        ...
        if (dbCheck.status === 'connected') {
          this.logTest('Database connection', true, `Response time: ${dbCheck.responseTime}`);

          if (dbCheck.tablesFound > 0) {
            this.logTest('Database schema exists', true, `Tables found: ${dbCheck.tablesFound}`);
          } else {
            this.logTest('Database schema exists', false, 'No tables found - may need migration');
          }
        } else {
          this.logTest(
            'Database connection',
