...
class AccessibilityTestRunner {
      ...
      const contrastIssues = await this.page.evaluate(() => {
        ...
        textElements.forEach(el => {
          ...
          if (
            ...
            if (color === 'white' || color === '#ffffff' || color.includes('rgba(255, 255, 255')) {
              issues.push({
  ...
  evaluateScenarioResults(scenarioResults) {
    ...
    return (
      violationCounts.critical <= A11Y_CONFIG.thresholds.critical &&
...
<body>
    ...
    await fs.writeFile(path.join(A11Y_CONFIG.outputDir, 'accessibility-report.html'), html);
  }
...
`;
    ...
    await fs.writeFile(path.join(A11Y_CONFIG.outputDir, 'accessibility-summary.md'), markdown);
  }
