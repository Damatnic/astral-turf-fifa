...
try {
    $health = Invoke-RestMethod -Uri "http://localhost:3333" -Method GET -ErrorAction Stop
    Write-Host "[PASS] Server is running" -ForegroundColor Green
} catch {
    Write-Host "[FAIL] Server is not running! Please start with: cd backend; npm run start:dev" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    if ($registerResponse.message -like "*check your email*") {
        Write-Host "`n[INFO] Verification email should be sent!" -ForegroundColor Cyan
...
Write-Host "`n[WAIT] Pausing for 2 seconds..." -ForegroundColor Gray
...
try {
    ...
    Write-Host "[WARN] Login succeeded (email verification may be disabled)" -ForegroundColor Yellow
    $accessToken = $loginResponse.accessToken
    $userId = $loginResponse.user.id
} catch {
    if ($_.Exception.Response.StatusCode -eq 403) {
        Write-Host "[PASS] Login correctly blocked - email not verified" -ForegroundColor Green
...
try {
    ...
    Write-Host "[PASS] Resend verification successful" -ForegroundColor Green
    Write-Host "   Message: $($resendResponse.message)" -ForegroundColor Gray
} catch {
    Write-Host "[INFO] Resend verification endpoint error" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "[PASS] Password reset request successful" -ForegroundColor Green
    Write-Host "   Message: $($forgotResponse.message)" -ForegroundColor Gray
    
    if ($forgotResponse.message -like "*If an account exists*") {
        Write-Host "   [PASS] Generic message prevents user enumeration" -ForegroundColor Green
    }
    
    Write-Host "`n[INFO] Password reset email should be sent!" -ForegroundColor Cyan
    Write-Host "   (Configure SMTP in .env to receive actual emails)" -ForegroundColor Yellow
} catch {
    Write-Host "[FAIL] Password reset request failed" -ForegroundColor Red
    Write-Host "   Error: $($_.Exception.Message)" -ForegroundColor Red
}
...
for ($i = 1; $i -le 4; $i++) {
    try {
        ...
        if ($_.Exception.Response.StatusCode -eq 429) {
            $rateLimitHit = $true
            Write-Host "   Request $i`: Rate limited (HTTP 429)" -ForegroundColor Yellow
        } else {
...
if ($rateLimitHit) {
    ...
    Write-Host "`n[WARN] Rate limiting may need configuration" -ForegroundColor Yellow
}
...
try {
    ...
    if ($_.Exception.Response.StatusCode -eq 400) {
        Write-Host "[PASS] Invalid token correctly rejected (HTTP 400)" -ForegroundColor Green
    } else {
        Write-Host "[WARN] Unexpected error: $($_.Exception.Response.StatusCode)" -ForegroundColor Yellow
    }
}
...
Write-Host "   [PASS] Email verification system" -ForegroundColor Green
Write-Host "   [PASS] Password reset request" -ForegroundColor Green
Write-Host "   [PASS] Rate limiting" -ForegroundColor Green
Write-Host "   [PASS] Invalid token rejection" -ForegroundColor Green
...
Write-Host "All core authentication endpoints are functional!`n" -ForegroundColor Green
