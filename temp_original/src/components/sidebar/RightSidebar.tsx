  ...
  return (
    <div>
      ...
      <div className="w-full bg-[var(--bg-tertiary)] rounded-full h-1.5">
        <div
          className="bg-[var(--accent-secondary)] h-1.5 rounded-full"
  ...
  return (
    <div>
      ...
      <div className="flex items-center mb-4">
        <div
          className="w-16 h-16 rounded-full flex items-center justify-center font-bold text-2xl mr-4 border-2 border-[var(--accent-primary)]"
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">
            Age: {player.age} | {playerRole?.name || 'Unknown Role'}
          </p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 mb-4">
        <div>
          <span className="text-sm font-semibold text-[var(--text-secondary)]">Morale:</span>
          <span className="text-sm font-bold ml-2 text-[var(--text-primary)]">{player.morale}</span>
        </div>
        <div>
          <span className="text-sm font-semibold text-[var(--text-secondary)]">Form:</span>
          <span className="text-sm font-bold ml-2 text-[var(--text-primary)]">{player.form}</span>
        </div>
      </div>
        ...
        <div className="mt-4 p-3 bg-[var(--bg-secondary)] rounded-md">
          ...
          <div className="flex flex-wrap gap-2">
            {player.traits.map(trait => (
              <span
                key={trait}
                className="text-xs font-semibold bg-purple-500/20 text-purple-300 px-2 py-1 rounded-full flex items-center"
              >
        ...
        <div className="mt-4 p-3 bg-[var(--bg-secondary)] rounded-md">
          ...
          <div className="flex flex-wrap gap-2">
            {playerInstructions.map(instr => (
              <span
                key={instr}
                className="text-xs font-semibold bg-teal-500/20 text-teal-300 px-2 py-1 rounded-full"
              >
                {instr}
              </span>
            ))}
...
const AIInsights: React.FC = () => {
  ...
  const handleGenerateInsights = async () => {
    ...
    try {
      ...
      const insight = await getTacticalAdvice(
        homePlayers,
        awayPlayers,
        homeFormation,
        awayFormation,
    ...
    <div>
      <div className="mb-3">
        <p className="text-xs text-[var(--text-secondary)] mt-1">
          Analyzes the Home team's ({homeFormation.name}) strategy against the Away team's (
          {awayFormation.name}).
        </p>
      </div>
      <button
        ...
        <button
          ...
          className={`w-full text-xs font-semibold px-2 py-1 rounded-md transition-colors ${
            value === option.value
              ...
              : 'text-[var(--text-secondary)] hover:bg-gray-600/70'
          }`}
        >
          {option.label}
        </button>
...
const TeamTacticsEditor: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-[var(--text-secondary)] mt-1">
        Set the high-level strategy for the{' '}
        <span className={`font-bold ${activeTeam === 'home' ? 'text-blue-400' : 'text-red-400'}`}>
          {activeTeam} team
        </span>
        .
      </p>
...
const TabButton: React.FC<{
  ...
  <button
    onClick={onClick}
    title={label}
    className={`flex-1 flex justify-center items-center p-2 transition-colors ${active ? 'bg-[var(--bg-secondary)] text-[var(--accent-primary)]' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)]'}`}
  >
    {icon}
  </button>
);
