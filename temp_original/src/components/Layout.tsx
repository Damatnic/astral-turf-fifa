...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div
      className={`flex flex-col h-screen w-screen bg-[var(--bg-primary)] text-[var(--text-primary)] overflow-hidden font-sans transition-colors duration-300 ${theme}`}
    >
      {/* Header */}
      {!isPresentationMode && <Header />}

      {/* Main Content Area */}
      <main className="flex flex-grow overflow-hidden">{children ? children : <Outlet />}</main>

      {/* Footer (could be extended with additional footer content) */}
      <footer className="sr-only">{/* Footer content can be added here if needed */}</footer>
