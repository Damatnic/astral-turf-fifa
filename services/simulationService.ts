...
import { Player, TeamTactics, MatchEvent, Team, PlayerRole, PositionRole, PlayerStats, RootState, MentoringGroup, MatchCommentary, MatchResult } from '../types';
    ...
    for (let minute = 1; minute <= 90; minute++) {
        ...
        if (Math.random() < chanceCreation) {
            ...
            if (Math.random() < goalProbability) {
                ...
                const scorer = scorers.length > 0 ? scorers[Math.floor(Math.random() * scorers.length)] : attackers[0];
                ...
                const event: MatchEvent = { minute, type: 'Goal', team: possession, playerName: scorer.name, description: assister ? `scored a goal, assisted by ${assister.name}.` : 'scored a goal.', assisterName: assister?.name };
                events.push(event);
                onUpdate(event);
